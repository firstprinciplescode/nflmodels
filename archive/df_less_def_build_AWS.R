...
# Get all defenses with >= 10 games
...
for(i in 1:nrow(query_less)) {
  ...
  # Get defense for this game
  rel_ssn <- query_less[i, ]$def_ssn
  
  # Remove THIS GAME (not all games for this defense)
  ...
  if(nrow(def_good_row) > 0 && nrow(def_bad_row) > 0 && nrow(def_diff_row) > 0) {
    combined_row <- cbind(def_good_row, def_bad_row, def_diff_row) %>%
...
# put_object(
#   file = "df_less_def_build_workspace_AWS.RData",
#   object = "temp/df_less_def_build_workspace_AWS.RData",
...
comparison_less_def_func("PHI2025", .95)
