...
query_less <- query_less %>%
  arrange(qbgrp_ssn, week, season)

# Leave-one-QB-out loop
unique_qbs <- unique(query_less$qbgrp_ssn)

# xtd_less_df <- data.frame()
for(i in 1:nrow(query_less)) {
  
  if(i %% 100 == 0) cat("Processing game", i, "of", nrow(query_less), "\n")
  
  # Get QB for this game
  rel_ssn <- query_less[i, ]$qbgrp_ssn
  
  # Remove THIS GAME (not all games for this QB)
  df_press <- query_less[-i, ]
  ...
  if(nrow(qb_good_row) > 0 && nrow(qb_bad_row) > 0 && nrow(qb_diff_row) > 0) {
    ...
    xtd_less_df <- rbind(xtd_less_df, combined_row)
...
cbind(xtd_less_df, query_less %>% select(team_name, week, season)) %>%
