# ============================================================================
# BLITZ OFFENSE BUILD - FAST VERSION
# ============================================================================
...
query_blitz <- query_blitz %>% arrange(qbgrp_ssn, week, season)
...
#   df_press <- query_blitz[-i, ]  # removes row, recalculates EVERYTHING
...
all_good_means <- query_blitz %>%
  filter(good_off_ind == 'Good') %>%
  group_by(qbgrp_ssn) %>%
  summarise(across(all_of(numeric_cols), mean, na.rm = TRUE), .groups = 'drop')

all_bad_means <- query_blitz %>%
  filter(good_off_ind == 'Bad') %>%
  group_by(qbgrp_ssn) %>%
  summarise(across(all_of(numeric_cols), mean, na.rm = TRUE), .groups = 'drop')

cat("Processing", nrow(query_blitz), "games with fast LOO...\n")

xtd_depth_df <- map_dfr(1:nrow(query_blitz), function(i) {
  
  if(i %% 500 == 0) cat("Processing game", i, "of", nrow(query_blitz), "\n")
...
save.image("~/df_blitz_off_build_workspace_AWS.RData")
