# ============================================================================
# PASSING TIP (LESS) OFFENSE BUILD - FAST VERSION
# ============================================================================
...
query_less <- query_less %>% arrange(qbgrp_ssn, week, season)
...
# for(i in 1:nrow(query_less)) {
#   df_press <- query_less[-i, ]  # removes row, recalculates EVERYTHING
...
cat("\nCOMPLETE - Created", nrow(xtd_less_df), "game feature rows\n")
...
task_before_xtd_no_temp <- prepare_task(xtd_less_df_train_2, "less_characteristics")
...
df_less_scaled_z <- xtd_less_df %>%
  select(-c(team_name, week, season)) %>%
  group_by(qbgrp_ssn_Good) %>%
  summarise(across(everything(), mean, na.rm = TRUE)) %>%
  rename(qbgrp_ssn = qbgrp_ssn_Good)

importance_matrix_less <- importance_no_temp

comparison_less_func <- function(qbgrp_ssn2, threshold, year = NULL) {
  relevant_features <- importance_matrix_less$Feature
  relevant_features <- relevant_features[relevant_features %in% colnames(df_less_scaled_z)]
...
# Test it
comparison_less_func("PHIHurts-2025", .9)

# Save dependencies
list_dependencies_less <- list(
  df_less_scaled_z = df_less_scaled_z,
  importance_matrix_less = importance_matrix_less
)

save(list_dependencies_less, file = "list_dependencies_less.RData")
put_object(
  file = "list_dependencies_less.RData",
  object = "qbgrp_def_functions/list_dependencies_less.RData",
  bucket = "nfl-pff-data-lucas"
)

save(comparison_less_func, file = "comparison_less_func.rds")
put_object(
  file = "comparison_less_func.rds",
  object = "qbgrp_def_functions/comparison_less_func.rds",
  bucket = "nfl-pff-data-lucas"
)

save.image("~/df_less_build_workspace_AWS.RData")