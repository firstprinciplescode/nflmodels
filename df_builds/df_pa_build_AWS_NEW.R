# ============================================================================
# PA (PLAY ACTION) OFFENSE BUILD - FAST VERSION
# ============================================================================
# NOTE: Fixed bug - max_blitz should be max_depth (copy-paste error)
# NOTE: Original had loop starting at 2057 (resume point) - fixed to start at 1
# ============================================================================
...
# for(i in 1:nrow(query_pa)) {
#   df_press <- query_pa[-i, ]  # removes row, recalculates EVERYTHING
#   df_good <- df_press %>% filter(...) %>% group_by(...) %>% summarise(...)
#   ... slow ...
# }
#
...
all_good_means <- query_pa %>%
  filter(good_off_ind == 'Good') %>%
  group_by(qbgrp_ssn) %>%
  summarise(across(all_of(numeric_cols), mean, na.rm = TRUE), .groups = 'drop')

all_bad_means <- query_pa %>%
  filter(good_off_ind == 'Bad') %>%
  group_by(qbgrp_ssn) %>%
  summarise(across(all_of(numeric_cols), mean, na.rm = TRUE), .groups = 'drop')

cat("Processing", nrow(query_pa), "games with fast LOO...\n")

xtd_less_df <- map_dfr(1:nrow(query_pa), function(i) {
  
  if(i %% 500 == 0) cat("Processing game", i, "of", nrow(query_pa), "\n")
...
xtd_pa_df <- cbind(xtd_less_df, query_pa %>% select(team_name, week, season))

xtd_pa_df_train <- left_join(
  xtd_pa_df, 
  xtd_training %>% select(posteam:season, mixed_xtd), 
  by = c("team_name" = "posteam", "week" = "week", "season" = "season")
) %>% filter(!is.na(mixed_xtd))

xtd_pa_df_train_2 <- xtd_pa_df_train %>%
...
task_before_xtd_no_temp <- prepare_task(xtd_pa_df_train_2, "pa_characteristics")
...
final_model_no_temp <- train_final_model(xtd_pa_df_train_2, tuning_before_xtd_no_temp$best_params)
...
df_pa_scaled_z <- xtd_pa_df %>%
  select(-c(team_name, week, season)) %>%
  group_by(qbgrp_ssn_Good) %>%
  summarise(across(everything(), mean, na.rm = TRUE)) %>%
  rename(qbgrp_ssn = qbgrp_ssn_Good)

importance_matrix_pa <- importance_no_temp

comparison_pa_func <- function(qbgrp_ssn2, threshold, year = NULL) {
  relevant_features <- importance_matrix_pa$Feature
  relevant_features <- relevant_features[relevant_features %in% colnames(df_pa_scaled_z)]
...
# Test it
comparison_pa_func("PHIHurts-2025", 1)

# Save dependencies
list_dependencies_pa <- list(
  df_pa_scaled_z = df_pa_scaled_z,
  importance_matrix_pa = importance_matrix_pa
)

save(list_dependencies_pa, file = "list_dependencies_pa.RData")
put_object(
  file = "list_dependencies_pa.RData",
  object = "qbgrp_def_functions/list_dependencies_pa.RData",
  bucket = "nfl-pff-data-lucas"
)

save(comparison_pa_func, file = "comparison_pa_func.rds")
put_object(
  file = "comparison_pa_func.rds",
  object = "qbgrp_def_functions/comparison_pa_func.rds",
  bucket = "nfl-pff-data-lucas"
)

save.image("~/df_pa_off_build_workspace_AWS.RData")
