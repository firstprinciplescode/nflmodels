...
# -- no model.
#
# !! UNSIGNED GATES IN FORCE !!
# The Phase-1 OL canon file is not in the session record Kimi holds,
# so canon X/G could not be imported. Constants below are
# PROPOSED -- Kimi, UNSIGNED (basis cited inline). Andy stamps or
# corrects, then re-source whole file. One place, per the law.
# 2025 smell test under the proposed gates (from the port run):
# Brewer / Lindstrom / Wirfs / Sewell / T. Smith / A. Thomas /
# McKivitz / Bortolini / Nelson / Meinerz -- reads like an All-Pro
...
# the overall run-block grade/snap pair (confirmed present on the
# frame -- the intake wall checked). If a future frame ever lacks
# them, the wall prints every run_block column and STOPs -- no
# substitution. A scheme-split (gap c3 / zone c3) is a round-two
# knob and would need a new ruling.
#
...
# ------------------------------------------------------------
# 1. constants -- PROPOSED -- Kimi, UNSIGNED (see header)
# ------------------------------------------------------------

X_QUAL_RBLK <- 8        # PROPOSED: qualifying game = >= 8 overall
# run-block snaps. Basis: midpoint of Andy's
# exploration floors (gap >= 7 / zone >= 9).
G_MIN_RBLK  <- 6        # PROPOSED: house universal (every canon unit
# runs G = 6).
...
cat("\n--- eyeball: 2025 top-10 by c3",
    "(UNSIGNED gates -- smell test before stamp) ---\n")
...
legend_rblk <- c(
  ...
  "snaps (PROPOSED floor -- UNSIGNED until Andy stamps):",
...
# system('aws s3 cp ~/run_blocking_c3_workspace.RData s3://nfl-pff-data-lucas/workspaces/')