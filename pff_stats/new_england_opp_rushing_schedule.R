...
# ------------------------------------------------------------
# 2. RUN DEFENSE, PER TEAM -- the banded unit at team grain.
#    Faced side rebuilt per (opponent, band) at the run-D gate
#    literals (3 run snaps / 6 games); 2026 side = team_band_2026.
# ------------------------------------------------------------

faced_rd_team_2025 <- run_defense_qbgrp %>%
  filter(startsWith(qbgrp_ssn, "NE"), season == 2025, in_season(week),
         position %in% c("DI", "ED", "LB", "S"), snap_counts_run >= 3) %>%
  transmute(player_id,
...
rd_team_cmp <- team_band_2026 %>%
  left_join(faced_rd_team_2025 %>% select(team_name, band, grade_25, stop_25),
            by = c("team_name", "band")) %>%
  ...
  ungroup() %>%
  arrange(desc(team_d), match(band, c("DI", "ED", "LB", "S")))

cat("\n--- RUN-D PER TEAM x BAND (team_d = mean grade delta, ordering only) ---\n")
print(rd_team_cmp %>% select(team_name, band, grade_25, grade_26, d_grade,
                             stop_25, stop_26, d_stop, interp_share), n = Inf)

rundef_team_gt <- rd_team_cmp %>%
  ...
  data_color(columns = c(gd, sd),
             fn = scales::col_numeric(c("#6baed6", "#f7f7f7", "#C60C30"),
                                      domain = c(-max(abs(c(rd_team_cmp$d_grade,
                                                            rd_team_cmp$d_stop)), na.rm = TRUE),
                                                 max(abs(c(rd_team_cmp$d_grade,
                                                           rd_team_cmp$d_stop)), na.rm = TRUE))),
             autocolor_text = TRUE) %>%
...
# heatmap: team x band, fill = delta, grade / stop facets
rd_hm_pd <- rd_team_cmp %>%
  select(team_name, band, d_grade, d_stop, team_d) %>%
  pivot_longer(c(d_grade, d_stop), names_to = "metric", values_to = "delta") %>%
  mutate(team_name = factor(team_name,
                            levels = rd_team_cmp %>% arrange(team_d) %>%
                              distinct(team_name) %>% pull(team_name)),
         band   = factor(band, levels = c("DI", "ED", "LB", "S")),
...
hm_dom <- max(abs(rd_hm_pd$delta), na.rm = TRUE)

plot_rundef_team_hm <- ggplot(rd_hm_pd, aes(x = band, y = team_name,
                                            fill = delta)) +
  geom_tile(color = "white", linewidth = 0.6) +
  geom_text(aes(label = lab), size = 2.8) +
  facet_wrap(~ metric, nrow = 1) +
  scale_fill_gradient2(low = "#6baed6", mid = "#f7f7f7", high = "#C60C30",
                       midpoint = 0, limits = c(-hm_dom, hm_dom),
...
# ------------------------------------------------------------
# 3. PASS RUSH, PER TEAM -- object hunt. Paste this whole block
#    back; the prush per-team table + heatmap get written against
#    the REAL object/column names, not guessed ones.
# ------------------------------------------------------------

cat("\n--- PRUSH OBJECT HUNT (paste back) ---\n")
cand <- grep("prush|pass_r|passr", ls(), ignore.case = TRUE, value = TRUE)
if (length(cand) == 0) {
  cat("no prush-named objects; widening the net:\n")
  cand <- grep("slate|band|pctl|team_", ls(), ignore.case = TRUE, value = TRUE)
}
print(cand)
for (o in cand) {
  obj <- get(o)
  if (is.data.frame(obj)) {
    cat("\n", o, "--", nrow(obj), "rows x", ncol(obj), "cols:\n")
    print(head(colnames(obj), 40))
  } else {
    cat("\n", o, "--", class(obj)[1], "len", length(obj), "\n")
    if (is.atomic(obj) && length(obj) <= 40) print(obj)
  }
}
