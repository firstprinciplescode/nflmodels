...
#           no imputation"): every split-gated defender who PLAYED
#           vs the focal gets a percentile -- earned 2025 currency
#           pctl if season-gated, else the v3 ARM prior for his
#           split x band (rookie_grade if entry_year == 2025, else
#           vet_grade; pr_supp / pr_pbu ride for context facets).
#           NOBODY drops from the faced denominator. fill_share =
#           share of faced snaps priced at priors -- the honesty
#           number (replaces unscored_share). The Echols ruling,
#           PIT/CB/zone drill. The X game-inclusion gate stands:
#           a cameo under X split snaps is still not "playing".
#
...
# ------------------------------------------------------------
# 9. FACED 2025 SIDE PER SPLIT -- FACED FILL LAW (08-17):
#    every SPLIT-gated coverage game vs NE's PASSING game
#    (qbgrp_ssn), weighted by that game's SPLIT snaps, priced at
#    the earned 2025 pctl if season-gated, else the v3 arm prior.
#    fill_share = the honesty number, per split.
...
faced_games_one_mz <- function(sp) {
  ...
  secondary_qbgrp_mz %>%
    ...
    mutate(arm  = if_else(!is.na(entry_year) & entry_year == 2025,
                          "rookie", "vet"),
           fill = is.na(g25),
           g25  = dplyr::coalesce(g25, if_else(arm == "rookie",
                                               rookie_grade, vet_grade)),
           s25  = dplyr::coalesce(s25, pr_supp),
           p25  = dplyr::coalesce(p25, pr_pbu)) %>%
    select(player_id, player, team, week, band, g_snaps,
           g25, s25, p25, arm, fill, split)
}
...
faced_band_mz <- faced_games_mz %>%
  ...
  summarise(defender_games = dplyr::n(),
            ...
            supp_25  = weighted.mean(s25, w = g_snaps, na.rm = TRUE),
            pbu_25   = weighted.mean(p25, w = g_snaps, na.rm = TRUE),
            .groups = "drop")

cat("\n--- faced 2025 band profile PER SPLIT (fill_share = honesty) ---\n")
...
#                which snap weights (and how many prior-filled)
...
##
## ============================================================================
## BUILT-IN TEST -- COVERAGE (man/zone) LEAGUE DIFFERENTIAL + NE REPRODUCTION WALL
...
needed_sec <- c("coverage_qbgrp_mz", "CN", "CM", "CZ",
                "combined_ids_defense", "pff_team_lookup",
                "blend2", "percent_rank_avg",
                "cmp_slate_mz", "cmp_unit_mz")
missing_sec <- needed_sec[!vapply(needed_sec, exists, logical(1))]
if (length(missing_sec)) stop("missing: ", paste(missing_sec, collapse = ", "),
                              " -- source the 6b build file first (the tower)")

# Andy's ruling layer is a raw input too; absent = empty (full sign-off)
if (!exists("opp_cov_2026_deltas_mz")) {
  opp_cov_2026_deltas_mz <- tibble::tibble(
...
coverage_qbgrp_mz    <- tibble::as_tibble(coverage_qbgrp_mz)
combined_ids_defense <- tibble::as_tibble(combined_ids_defense)
cmp_slate_mz         <- tibble::as_tibble(cmp_slate_mz)
cmp_unit_mz          <- tibble::as_tibble(cmp_unit_mz)

# column-map integrity gate: the tower's resolved names must be live
core_need_sec <- c("grade", "snaps", "tgt", "rec", "pbu")
map_chk <- c(CN$grade, CN$snaps,
             CM[core_need_sec], CZ[core_need_sec])
if (any(is.na(map_chk)) || !all(map_chk %in% names(coverage_qbgrp_mz))) {
  cat("resolved columns missing from coverage_qbgrp_mz:\n")
  print(map_chk[!(map_chk %in% names(coverage_qbgrp_mz))])
  stop("tower column maps do not resolve against the feed -- re-source 6b")
}

X_MAN_SEC  <- 6;   G_MAN_SEC  <- 6L
X_ZONE_SEC <- 13;  G_ZONE_SEC <- 6L
BANDS_SEC_LG <- c("CB", "SCB", "S", "LB")
N_SEC_LG     <- c(CB = 3L, SCB = 1L, S = 3L, LB = 2L)
...
lookup_patch_sec <- c("ARI" = "ARZ", "AZ" = "ARZ", "BAL" = "BLT",
                      "CLE" = "CLV", "HOU" = "HST", "LAR" = "LA",
                      "JAC" = "JAX", "WSH" = "WAS")
to_pff_sec <- function(x) unname(dplyr::coalesce(pff_team_lookup[x],
                                                 lookup_patch_sec[x], x))

# ------------------------------------------------------------
# 1. BASE FRAME + MODAL BAND (UN-GATED, combined snaps) -- canon law
# ------------------------------------------------------------

sec_qbgrp_lg <- coverage_qbgrp_mz %>%
  mutate(band4 = dplyr::case_when(
    final_position == "SCB"            ~ "SCB",
    final_position %in% c("MLB", "LB") ~ "LB",
    TRUE ~ final_position)) %>%
  filter(band4 %in% BANDS_SEC_LG,
         !is.na(.data[[CN$grade]]),
         .data[[CN$snaps]] > 0)

# old-code guard (canon bad_tm wall)
bad_tm_sec <- c("ARI", "BAL", "CLE", "HOU")
stopifnot(!any(sec_qbgrp_lg$team %in% bad_tm_sec))

dup_sec <- sec_qbgrp_lg %>%
  dplyr::count(player_id, week, season) %>%
  filter(n > 1)
if (nrow(dup_sec)) {
  print(head(as.data.frame(dup_sec), 10))
  stop("fan-out: ", nrow(dup_sec), " player-weeks with >1 row")
}

modal_band_sec_lg <- sec_qbgrp_lg %>%
  dplyr::count(player_id, season, band4, wt = .data[[CN$snaps]], name = "sn") %>%
  group_by(player_id, season) %>%
  slice_max(sn, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(player_id, season, band = band4)

multi_band_sec <- sec_qbgrp_lg %>%
  distinct(player_id, season, band4) %>%
  dplyr::count(player_id, season) %>%
  filter(n > 1)
cat("[0] band-identity tripwire: player-seasons spanning >1 band4",
    "(consolidated to modal):", nrow(multi_band_sec), "\n")

# ------------------------------------------------------------
# 2. SPLIT CURRENCY BUILDER (lean: grade / cmp_rt / pbu_rt only --
#    pctls of these do not depend on the dropped context columns)
# ------------------------------------------------------------

cur_build_sec <- function(split) {
  C <- if (split == "man") CM else CZ
  x <- if (split == "man") X_MAN_SEC else X_ZONE_SEC
  g <- if (split == "man") G_MAN_SEC else G_ZONE_SEC
  sec_qbgrp_lg %>%
    filter(.data[[C["snaps"]]] >= x) %>%
    ...
    summarise(
      ...
      q_snaps = sum(.data[[C["snaps"]]], na.rm = TRUE),
      q_tgts  = sum(.data[[C["tgt"]]],   na.rm = TRUE),
      grade   = weighted.mean(.data[[C["grade"]]],
                              w = .data[[C["snaps"]]], na.rm = TRUE),
      pbu_rt  = sum(.data[[C["pbu"]]], na.rm = TRUE) / pmax(q_tgts, 1),
      cmp_rt  = sum(.data[[C["rec"]]], na.rm = TRUE) / pmax(q_tgts, 1),
      .groups = "drop") %>%
    filter(q_games >= g) %>%
    mutate(split = split)
}

cov_pctl_sec_lg <- bind_rows(cur_build_sec("man"), cur_build_sec("zone")) %>%
  group_by(split, band, season) %>%
  mutate(grade_pctl = percent_rank_avg(grade),
         supp_pctl  = percent_rank_avg(-cmp_rt),   # INVERTED
         pbu_pctl   = percent_rank_avg(pbu_rt)) %>%
  ungroup()
# NaN honesty pass (all-NA cells)
for (cc in c("grade_pctl", "supp_pctl", "pbu_pctl"))
  cov_pctl_sec_lg[[cc]][is.nan(cov_pctl_sec_lg[[cc]])] <- NA_real_

cat("[0] gated player-seasons, split x band, 2025 pools:\n")
print(cov_pctl_sec_lg %>% filter(season == 2025) %>%
        dplyr::count(split, band) %>% arrange(split, band) %>% as.data.frame())

# ------------------------------------------------------------
# 3. USAGE / BAND MAP / ENTRY YEARS / CROSSWALK / ROSTERS
# ------------------------------------------------------------

use_one_sec <- function(split) {
  C <- if (split == "man") CM else CZ
  x <- if (split == "man") X_MAN_SEC else X_ZONE_SEC
  sec_qbgrp_lg %>%
    filter(.data[[C["snaps"]]] >= x, season %in% c(2024, 2025)) %>%
    ...
    tidyr::pivot_wider(names_from = season,
                       values_from = c(cov_snaps, qg)) %>%
    mutate(split = split)
}
use_sec_lg <- bind_rows(use_one_sec("man"), use_one_sec("zone"))

band_map_sec_lg <- modal_band_sec_lg %>% filter(season == 2025) %>%
  select(player_id, band_25 = band) %>%
  full_join(modal_band_sec_lg %>% filter(season == 2024) %>%
              select(player_id, band_24 = band),
            by = "player_id") %>%
  mutate(band = dplyr::coalesce(band_25, band_24)) %>%
  select(player_id, band)

xw_sec_lg <- combined_ids_defense %>%
  filter(!is.na(gsis_id)) %>%
  distinct(player_id, gsis_id)
stopifnot(anyDuplicated(xw_sec_lg$gsis_id)   == 0,
          anyDuplicated(xw_sec_lg$player_id) == 0)

ent_sec_lg <- nflreadr::load_rosters(2017:2025) %>%
  filter(!is.na(gsis_id), !is.na(entry_year)) %>%
  group_by(gsis_id) %>%
  summarise(entry_year = min(entry_year), .groups = "drop") %>%
  inner_join(xw_sec_lg, by = "gsis_id") %>%
  select(player_id, entry_year)

cov_roster_pos_sec <- c("CB", "S", "FS", "SS", "SAF", "DB", "NB", "SCB",
                        "LB", "ILB", "MLB", "OLB")
ros_sec_lg <- nflreadr::load_rosters(2026) %>%
  filter(position %in% cov_roster_pos_sec) %>%
  transmute(gsis_id,
            roster_name = full_name,
            team = to_pff_sec(team),
            entry_year,
            roster_pos = position) %>%
  left_join(xw_sec_lg, by = "gsis_id")
stopifnot(anyDuplicated(ros_sec_lg[, c("team", "roster_name")]) == 0)

# slot curves per split (2025, split-gated, G-filtered) -- pff32 from these
slot_one_sec <- function(split) {
  C <- if (split == "man") CM else CZ
  x <- if (split == "man") X_MAN_SEC else X_ZONE_SEC
  g <- if (split == "man") G_MAN_SEC else G_ZONE_SEC
  sec_qbgrp_lg %>%
    ...
    ungroup() %>%
    mutate(split = split) %>%
    select(split, team, band, slot_rk, sn_slot = sn)
}
slot_sec_lg <- bind_rows(slot_one_sec("man"), slot_one_sec("zone"))

slot_med_sec_lg <- slot_sec_lg %>%
  group_by(split, band, slot_rk) %>%
  summarise(sn_med = median(sn_slot), .groups = "drop")
fb_sec_lg <- slot_sec_lg %>%
  group_by(split) %>%
  summarise(fb = min(sn_slot), .groups = "drop")

pff32_sec <- sort(unique(slot_sec_lg$team))
stopifnot(length(pff32_sec) == 32L)

# roster code wall BEFORE any filtering
ros_teams <- sort(unique(ros_sec_lg$team))
if (!setequal(ros_teams, pff32_sec)) {
  cat("roster-vs-currency code mismatch:\n")
  cat("roster-only:", setdiff(ros_teams, pff32_sec), "\n")
  cat("currency-only:", setdiff(pff32_sec, ros_teams), "\n")
  stop("team code alignment failed (rosters)")
}

## ============================================================================
## 4. PRIOR AUDIT PER SPLIT (v3, verbatim port of 6b block 5)
##    promotion class = priced committee members (top-N by split usage
##    within team x band x season) realized in t with NO gated split
##    season in t-1/t-2. rungs: arm n>=10 -> pooled n>=10 -> v2/S-splitCB.
## ============================================================================

committee_hist_one_sec <- function(split) {
  C <- if (split == "man") CM else CZ
  x <- if (split == "man") X_MAN_SEC else X_ZONE_SEC
  sec_qbgrp_lg %>%
    filter(.data[[C["snaps"]]] >= x) %>%
    ...
    arrange(desc(sn), .by_group = TRUE) %>%
    mutate(band_rank = dplyr::row_number()) %>%
    filter(band_rank <= N_SEC_LG[band]) %>%
    ungroup() %>%
    mutate(split = split)
}
committee_hist_sec_lg <- bind_rows(committee_hist_one_sec("man"),
                                   committee_hist_one_sec("zone"))

gated_ps_sec_lg <- cov_pctl_sec_lg %>% distinct(split, player_id, season)

promo_sec_lg <- committee_hist_sec_lg %>%
  filter(season >= 2018) %>%
  inner_join(cov_pctl_sec_lg %>%
               select(split, player_id, season, band,
                      grade_pctl, supp_pctl, pbu_pctl),
             by = c("split", "player_id", "season", "band")) %>%
  left_join(gated_ps_sec_lg %>%
              transmute(split, player_id, season = season + 1L, g1 = TRUE),
            by = c("split", "player_id", "season")) %>%
  left_join(gated_ps_sec_lg %>%
              transmute(split, player_id, season = season + 2L, g2 = TRUE),
...
cat("\n[4] promotion class sizes, 2018-2025, split x band x arm:\n")
print(promo_sec_lg %>% dplyr::count(split, band, arm) %>%
        arrange(split, band, arm) %>% as.data.frame())

promo_med_sec  <- promo_sec_lg %>%
  group_by(split, band, arm) %>%
  summarise(n = dplyr::n(),
            med_grade = median(grade_pctl, na.rm = TRUE),
            .groups = "drop")
promo_pool_sec <- promo_sec_lg %>%
  group_by(split, band) %>%
  summarise(n_pool    = dplyr::n(),
            pool_grade = median(grade_pctl, na.rm = TRUE),
            pool_supp  = median(supp_pctl,  na.rm = TRUE),
...
V2_LAW_SEC <- tribble(   # combined-currency prices, RUNG 3 only
  ~band, ~v2_rookie, ~v2_vet, ~v2_phantom,
  "CB", 0.433, 0.359, 0.408,
  "LB", 0.456, 0.338, 0.400,
  "S",  0.471, 0.465, 0.468)
...
cb_price_sec <- list()
for (sp in c("man", "zone")) {
  for (b in BANDS_SEC_LG) {   # CB first by construction
    pool <- promo_pool_sec %>% filter(split == sp, band == b)
    rook <- promo_med_sec  %>% filter(split == sp, band == b, arm == "rookie")
    vet  <- promo_med_sec  %>% filter(split == sp, band == b, arm == "vet")
    n_pool <- if (nrow(pool)) pool$n_pool else 0L
    n_rook <- if (nrow(rook)) rook$n else 0L
    n_vet  <- if (nrow(vet))  vet$n  else 0L
    pool_g <- if (nrow(pool)) pool$pool_grade else NA_real_
    v2r <- if (b == "SCB") NA_real_ else V2_LAW_SEC$v2_rookie[V2_LAW_SEC$band == b]
    v2v <- if (b == "SCB") NA_real_ else V2_LAW_SEC$v2_vet[V2_LAW_SEC$band == b]
    v2p <- if (b == "SCB") NA_real_ else V2_LAW_SEC$v2_phantom[V2_LAW_SEC$band == b]
    r3r <- if (b == "SCB") cb_price_sec[[sp]]$rook else v2r
    r3v <- if (b == "SCB") cb_price_sec[[sp]]$vet  else v2v
    r3p <- if (b == "SCB") cb_price_sec[[sp]]$phan else v2p
    r3l <- if (b == "SCB") "split CB (rung 3)" else "v2 combined (rung 3)"
    if (n_rook >= N_ARM_MIN_SEC) { g_r <- rook$med_grade; rung_r <- "arm (n>=10)" }
    else if (n_pool >= N_ARM_MIN_SEC) { g_r <- pool_g; rung_r <- "pooled band x split" }
    else { g_r <- r3r; rung_r <- r3l }
    if (n_vet >= N_ARM_MIN_SEC)  { g_v <- vet$med_grade; rung_v <- "arm (n>=10)" }
    else if (n_pool >= N_ARM_MIN_SEC) { g_v <- pool_g; rung_v <- "pooled band x split" }
    else { g_v <- r3v; rung_v <- r3l }
    if (n_pool >= N_ARM_MIN_SEC) {
      g_p <- pool_g; rung_p <- "pooled band x split"
      s_p <- pool$pool_supp; b_p <- pool$pool_pbu
    } else {
      g_p <- r3p; rung_p <- r3l
    ...
    if (b == "CB") cb_price_sec[[sp]] <- list(rook = g_r, vet = g_v, phan = g_p)
    prior_rows_sec[[paste(sp, b)]] <- tibble(
      split = sp, band = b,
      rookie_grade = g_r, vet_grade = g_v, phantom_grade = g_p,
      pr_supp = s_p, pr_pbu = b_p,
      n_rook = n_rook, n_vet = n_vet, n_pool = n_pool,
...
prior_law_sec_lg <- bind_rows(prior_rows_sec)

cat("\n[4] PRIOR LAW v3 (rebuilt locally; rungs printed):\n")
print(prior_law_sec_lg %>%
        select(split, band, rookie_grade, vet_grade, phantom_grade,
               n_rook, n_vet, n_pool, rung_rookie, rung_vet, rung_phantom) %>%
        as.data.frame())

## ============================================================================
## 5. LEDGERS PER SPLIT -> COMMITTEES -> PHANTOMS (canon code path)
## ============================================================================

ledger_base_one_sec <- function(split) {
  u   <- use_sec_lg %>% filter(split == .env$split)
  p25 <- cov_pctl_sec_lg %>% filter(season == 2025, split == .env$split) %>%
    select(player_id, grade_25 = grade_pctl, supp_25 = supp_pctl,
           pbu_25 = pbu_pctl)
  p24 <- cov_pctl_sec_lg %>% filter(season == 2024, split == .env$split) %>%
    select(player_id, grade_24 = grade_pctl, supp_24 = supp_pctl,
           pbu_24 = pbu_pctl)
  ros_sec_lg %>%
    left_join(u,   by = "player_id") %>%
    left_join(band_map_sec_lg, by = "player_id") %>%
    left_join(p25, by = "player_id") %>%
    left_join(p24, by = "player_id") %>%
    mutate(band = if_else(band %in% BANDS_SEC_LG, band, NA_character_),
           status = case_when(
             entry_year == 2026                            ~ "rookie",
             is.na(player_id)                              ~ "no_pff_id",
             !is.na(grade_25)                              ~ "has_2025_pctl",
...
ledger_base_sec_lg <- bind_rows(ledger_base_one_sec("man"),
                                ledger_base_one_sec("zone"))

ledger_sec_lg <- ledger_base_sec_lg %>%
  filter(status != "no_split_history") %>%
  mutate(usage_ord = pmax(dplyr::coalesce(cov_snaps_2025, 0),
                          dplyr::coalesce(cov_snaps_2024, 0))) %>%
  group_by(split, team, band) %>%
  arrange(desc(usage_ord), .by_group = TRUE) %>%
  mutate(band_rank = dplyr::row_number(),
         proposed  = dplyr::coalesce(
           !is.na(band) & band_rank <= N_SEC_LG[band], FALSE)) %>%
  ungroup()

proj_one_sec <- function(split) {
  p25 <- cov_pctl_sec_lg %>% filter(season == 2025, split == .env$split) %>%
    select(player_id, g25 = grade_pctl, s25 = supp_pctl, p25 = pbu_pctl)
  p24 <- cov_pctl_sec_lg %>% filter(season == 2024, split == .env$split) %>%
    select(player_id, g24 = grade_pctl, s24 = supp_pctl, p24 = pbu_pctl)
  law <- prior_law_sec_lg %>% filter(split == .env$split) %>%
    select(band, rookie_grade, vet_grade, phantom_grade, pr_supp, pr_pbu)
  slots <- slot_sec_lg %>% filter(split == .env$split) %>% select(-split)
  meds  <- slot_med_sec_lg %>% filter(split == .env$split) %>% select(-split)
  fb    <- fb_sec_lg %>% filter(split == .env$split) %>% pull(fb)
  
  adds_prepped <- opp_cov_2026_deltas_mz %>%
    filter(action == "add") %>%
    left_join(ledger_base_sec_lg %>% filter(split == .env$split) %>%
  ...
  rot_real <- ledger_sec_lg %>%
    filter(split == .env$split, proposed) %>%
  ...
  # (team, name) dup law catches the proposed-CB + added-S collision class
  stopifnot(!any(is.na(rot_real$band)),
            all(rot_real$band %in% BANDS_SEC_LG),
            anyDuplicated(rot_real[, c("team", "roster_name")]) == 0)
  ...
  rot_real <- rot_real %>%
    ...
    filter(band_rank <= N_SEC_LG[band]) %>%
    ungroup() %>%
    left_join(slots, by = c("team", "band", "band_rank" = "slot_rk")) %>%
    left_join(meds,  by = c("band", "band_rank" = "slot_rk")) %>%
    mutate(usage_w = dplyr::coalesce(usage_w, sn_slot, sn_med, fb)) %>%
    select(team, roster_name, band, status, prior_used,
           usage_w, grade_f, supp_f, pbu_f)
  ...
  auto_phantoms <- rot_real %>%
    ...
    tidyr::complete(team = pff32_sec, band = BANDS_SEC_LG,
                    fill = list(n_real = 0L)) %>%
    mutate(n_target = N_SEC_LG[band]) %>%
    filter(n_real < n_target) %>%
    mutate(slot_rk = purrr::map2(n_real, n_target, ~ seq(.x + 1L, .y))) %>%
    tidyr::unnest(slot_rk) %>%
    left_join(slots, by = c("team", "band", "slot_rk")) %>%
    left_join(meds,  by = c("band", "slot_rk")) %>%
    left_join(law,   by = "band") %>%
    transmute(team,
              roster_name = paste0("PHANTOM_", band, slot_rk),
  ...
  bind_rows(rot_real, auto_phantoms) %>% mutate(split = split)
}

rot_2026_sec_lg <- bind_rows(proj_one_sec("man"), proj_one_sec("zone"))

# every (team, band, split) is exactly its N deep, no exceptions
depth_chk_sec <- dplyr::count(rot_2026_sec_lg, split, team, band)
stopifnot(all(depth_chk_sec$n == N_SEC_LG[depth_chk_sec$band]),
          nrow(rot_2026_sec_lg) == 32L * sum(N_SEC_LG) * 2L)
cat("\n[5] rot_2026_sec_lg:", nrow(rot_2026_sec_lg),
    "rows | every (team, band, split) at exact depth | phantoms:",
    sum(rot_2026_sec_lg$status == "phantom"), "\n")

# band grain (canon grain -- feeds the NE slate wall) ...
team_band_sec_lg <- rot_2026_sec_lg %>%
  group_by(split, team, band) %>%
  summarise(n_members    = dplyr::n(),
            interp_share = sum(usage_w[prior_used]) / sum(usage_w),
            grade_26 = weighted.mean(grade_f, w = usage_w),
            ...
            pbu_26   = weighted.mean(pbu_f,   w = usage_w),
            .groups = "drop")
stopifnot(nrow(team_band_sec_lg) == 32L * length(BANDS_SEC_LG) * 2L)

# band weights per split: league 2025 gated SPLIT-snap shares (earned)
band_wt_sec_lg <- cov_pctl_sec_lg %>%
  filter(season == 2025) %>%
  group_by(split, band) %>%
  summarise(sn = sum(q_snaps), .groups = "drop") %>%
  group_by(split) %>%
  ...
  ungroup() %>%
  select(split, band, wt)
cat("[5] unit-composite band weights per split:\n")
print(as.data.frame(band_wt_sec_lg))

# ... and unit grain (band-share weighted grade composite -- canon law)
team_unit_sec_lg <- team_band_sec_lg %>%
  left_join(band_wt_sec_lg, by = c("split", "band")) %>%
  group_by(split, team) %>%
  summarise(unit_26 = weighted.mean(grade_26, w = wt),
            interp_share = weighted.mean(interp_share, w = wt),
            .groups = "drop")
stopifnot(nrow(team_unit_sec_lg) == 64L)

## ============================================================================
## 6. FACED 2025 PER SPLIT, INCLUDING PLAYOFFS -- PFF-NATIVE (2026-08-17)
##    identity: defender faced focal F's passing game in week w <=> the
##    row's qbgrp_ssn starts with F -- QB-GROUP level ("NEMaye-2025": team
##    code glued to the QB surname; 2026-08-18 census proved the format).
##    Canon's startsWith() parsing, extended to all 32 focals by anchored
##    prefix extraction. week = PFF week; in_season() (the canon function) includes
##    the playoff weeks (PFF 28/29/30/32). NO nflverse bridge on the faced
##    side: nothing to download, nothing to mistranslate. splits NEVER
##    recombined. NE WALL A below certifies vs cmp_slate_mz at 1e-8.
## ============================================================================

## -- faced games per split: every split-gated 2025 game, snap-weighted,
##    FACED FILL LAW (arm-priced) at comparison time -------------------------
faced_games_one_sec <- function(split) {
  C <- if (split == "man") CM else CZ
  x <- if (split == "man") X_MAN_SEC else X_ZONE_SEC
  law <- prior_law_sec_lg %>% filter(split == .env$split) %>%
    select(band, rookie_grade, vet_grade, pr_supp)
  sec_qbgrp_lg %>%
    filter(season == 2025, in_season(week), .data[[C["snaps"]]] >= x) %>%
    transmute(player_id, week,
              focal_raw = qbgrp_ssn,
              team, band_row = band4,
              g_snaps = .data[[C["snaps"]]]) %>%
    left_join(cov_pctl_sec_lg %>% filter(season == 2025, split == .env$split) %>%
                select(player_id, band_cur = band,
                       g25 = grade_pctl, s25 = supp_pctl),
              by = "player_id") %>%
    mutate(band = dplyr::coalesce(band_cur, band_row), split = split) %>%
    left_join(ent_sec_lg, by = "player_id") %>%
    left_join(law, by = "band") %>%
    mutate(arm  = if_else(!is.na(entry_year) & entry_year == 2025,
                          "rookie", "vet"),
...
faced_games_sec_lg <- bind_rows(faced_games_one_sec("man"),
                                faced_games_one_sec("zone"))
...
##    "2025$" was the wrong parse). The frame's own team column is
##    PFF-native and needs no surgery. The focal TEAM is the anchored
##    PFF-code prefix -- extracted against the PFF 32, LONGEST code first
##    so LAC beats LA -- with to_pff_sec as backstop. Full receipt prints;
##    walls stop loud with the raw strings if anything fails to resolve.
pff_re_sec <- paste0("^(", paste(pff32_sec[order(-nchar(pff32_sec))],
                                 collapse = "|"), ")")
code_fix_sec <- faced_games_sec_lg %>%
  distinct(focal_raw) %>%
  mutate(focal = to_pff_sec(stringr::str_extract(focal_raw, pff_re_sec))) %>%
  arrange(focal, focal_raw)
cat("focal census:", nrow(code_fix_sec), "distinct qbgrp groups (team+QB) ->",
    dplyr::n_distinct(code_fix_sec$focal), "focal teams\n")
print(code_fix_sec %>% as.data.frame())
no_code_sec <- code_fix_sec %>% filter(is.na(focal))
if (nrow(no_code_sec) > 0L) {
  cat("qbgrp values with NO PFF team prefix:\n")
  print(no_code_sec$focal_raw)
  stop("focal extraction failed -- receipt printed above")
}

faced_games_sec_lg <- faced_games_sec_lg %>%
  mutate(focal = to_pff_sec(stringr::str_extract(focal_raw, pff_re_sec)))

## -- WALLS: every focal is a real PFF code; NE faced exactly 21 weeks
##    in EACH split (canonical 2025 slate incl. playoffs) --------------------
bad_focal_sec <- setdiff(unique(faced_games_sec_lg$focal), pff32_sec)
if (length(bad_focal_sec) > 0L) {
  cat("focal codes NOT in the PFF 32 after mapping:", bad_focal_sec, "\n")
  cat("raw qbgrp_ssn codes behind them:",
      sort(unique(faced_games_sec_lg$focal_raw[faced_games_sec_lg$focal %in% bad_focal_sec])), "\n")
  stop("focal code alignment failed -- census printed above")
}
## -- NE week wall: the engine's focal == "NE" rows must reproduce canon's
##    OWN row set (startsWith("NE") + the same per-game split snap gates).
##    canon's "expect the 21" was a cat, never a wall: the per-game floors
##    (6 man / 13 zone snaps) legitimately ZERO OUT weeks where no opposing
##    coverage defender met the floor vs NE (zone-heavy weeks kill the man
##    diet and vice versa), so the true gated diet is < 21 per split.
##    Walled: row-set equality with canon, NOT a number canon never
##    enforced. The 1e-8 split x band wall below remains the certification.
ne_wk_sec <- faced_games_sec_lg %>% filter(focal == "NE") %>%
  group_by(split) %>%
  summarise(weeks_engine = list(sort(unique(week))), .groups = "drop")

ne_wk_canon_sec <- bind_rows(
  sec_qbgrp_lg %>%
    filter(season == 2025, in_season(week), startsWith(qbgrp_ssn, "NE"),
           .data[[CM["snaps"]]] >= X_MAN_SEC) %>%
    summarise(split = "man", weeks_canon = list(sort(unique(week)))),
  sec_qbgrp_lg %>%
    filter(season == 2025, in_season(week), startsWith(qbgrp_ssn, "NE"),
           .data[[CZ["snaps"]]] >= X_ZONE_SEC) %>%
    summarise(split = "zone", weeks_canon = list(sort(unique(week)))))

ne21_sec <- sec_qbgrp_lg %>%
  filter(season == 2025, in_season(week), startsWith(qbgrp_ssn, "NE")) %>%
  distinct(week) %>% pull(week) %>% sort()
cat("NE 2025 weeks in the frame (the 21 incl. playoffs):", length(ne21_sec), "\n")

ne_wk_chk_sec <- ne_wk_sec %>%
  inner_join(ne_wk_canon_sec, by = "split") %>%
  mutate(n_engine = lengths(weeks_engine), n_canon = lengths(weeks_canon),
         match = mapply(identical, weeks_engine, weeks_canon))
cat("NE faced weeks per split -- engine (prefix extraction) vs canon (startsWith):\n")
print(ne_wk_chk_sec %>%
        select(split, n_engine, n_canon, match) %>% as.data.frame())
for (sp in c("man", "zone")) {
  wk <- ne_wk_chk_sec$weeks_engine[ne_wk_chk_sec$split == sp][[1]]
  cat(sprintf("  %s: %d of %d NE weeks | gated out (no defender met the %s floor): %s\n",
              sp, length(wk), length(ne21_sec),
              if (sp == "man") "6-man-snap" else "13-zone-snap",
              paste(setdiff(ne21_sec, wk), collapse = ", ")))
}
stopifnot(all(ne_wk_chk_sec$match))

## -- per-focal x split x band (the NE wall grain) ---------------------------
faced_band_sec_lg <- faced_games_sec_lg %>%
  group_by(focal, split, band) %>%
  summarise(grade_25 = weighted.mean(g25, w = g_snaps, na.rm = TRUE),
            supp_25  = weighted.mean(s25, w = g_snaps, na.rm = TRUE),
            fill_share_25 = sum(g_snaps[fill]) / sum(g_snaps),
            .groups = "drop")

## -- per-focal x split UNIT (band-share weighted, canon pooling) ------------
faced_unit_sec_lg <- faced_band_sec_lg %>%
  left_join(band_wt_sec_lg, by = c("split", "band")) %>%
  group_by(focal, split) %>%
  summarise(unit_25 = if (all(is.na(grade_25))) NA_real_
            else weighted.mean(grade_25, w = wt, na.rm = TRUE),
            fill_share_25 = sum(fill_share_25 * wt),
            .groups = "drop")

wk_diet_sec <- faced_games_sec_lg %>%
  distinct(focal, week) %>%
  group_by(focal) %>%
  summarise(n_weeks = dplyr::n(),
            po_weeks = sum(week > 18), .groups = "drop")
if (nrow(wk_diet_sec) != 32L) {
  cat("expected 32 focals, got", nrow(wk_diet_sec), "\n")
  cat("missing:", setdiff(pff32_sec, wk_diet_sec$focal), "\n")
  cat("extra:", setdiff(wk_diet_sec$focal, pff32_sec), "\n")
  stop("focal count wall failed -- census printed above")
}
cat("playoff weeks in each focal's faced diet:\n")
print(wk_diet_sec %>% arrange(desc(po_weeks), focal) %>% as.data.frame())

## -- NE WALL A: playoff-inclusive faced == shipped slate, split x band -----
gate_ne_sec <- faced_band_sec_lg %>%
  filter(focal == "NE") %>%
  inner_join(cmp_slate_mz %>%
               select(split, band, g_canon = grade_25, s_canon = supp_25,
                      u_canon = fill_share_25),
             by = c("split", "band")) %>%
  mutate(dg = abs(grade_25 - g_canon), ds = abs(supp_25 - s_canon),
         du = abs(fill_share_25 - u_canon))
stopifnot(nrow(gate_ne_sec) == 2L * length(BANDS_SEC_LG))
cat("NE faced wall: max |d grade| =", signif(max(gate_ne_sec$dg), 3),
    "| max |d supp| =", signif(max(gate_ne_sec$ds), 3),
    "| max |d fill| =", signif(max(gate_ne_sec$du), 3), "\n")
if (max(gate_ne_sec$dg, gate_ne_sec$ds, gate_ne_sec$du) > 1e-8) {
  print(as.data.frame(gate_ne_sec))
  stop("NE faced wall failed -- playoff-inclusive faced does not reproduce canon")
}
cat("NE WALL A PASS: faced side reproduces cmp_slate_mz at 1e-8 (8 cells)\n")

## -- RECEIPT: what the REG-only convention was costing ---------------------
ne_sec_reg <- faced_games_sec_lg %>%
  ...
  left_join(band_wt_sec_lg, by = c("split", "band")) %>%
  group_by(split) %>%
  summarise(unit_25 = weighted.mean(grade_25, w = wt, na.rm = TRUE),
            .groups = "drop")
ne_sec_po <- faced_unit_sec_lg %>% filter(focal == "NE")
cat("NE faced unit BEFORE/AFTER the playoff repair:\n")
for (sp in c("man", "zone")) {
  cat(sprintf("  %s: REG-only (WRONG) unit_25=%.4f -> incl. playoffs %.4f\n",
              sp, ne_sec_reg$unit_25[ne_sec_reg$split == sp],
              ne_sec_po$unit_25[ne_sec_po$split == sp]))
}

## ============================================================================
## 7. SLATE 2026 PER SPLIT (REG ONLY -- 2026 playoff opponents unknowable)
## ============================================================================
...
load_sched_sec <- function(season) {
...
sched26_teams <- sort(unique(to_pff_sec(c(sch26_sec$home_team,
                                          sch26_sec$away_team))))
if (!setequal(sched26_teams, pff32_sec)) {
  cat("2026 schedule-vs-currency code mismatch:\n")
  cat("schedule-only:", setdiff(sched26_teams, pff32_sec), "\n")
  cat("currency-only:", setdiff(pff32_sec, sched26_teams), "\n")
  stop("2026 schedule code alignment failed")
}

opp26_sec <- bind_rows(
  sch26_sec %>% transmute(focal = home_team, opp = away_team),
  sch26_sec %>% transmute(focal = away_team, opp = home_team)) %>%
  mutate(focal = to_pff_sec(focal), opp = to_pff_sec(opp))
opp_n_sec <- opp26_sec %>% dplyr::count(focal, name = "n_games")
stopifnot(nrow(opp26_sec) == 544L, all(opp_n_sec$n_games == 17L))

## band grain (canon grain -- the NE slate wall) ...
slate_band_sec_lg <- opp26_sec %>%
  inner_join(team_band_sec_lg, by = c("opp" = "team"),
             relationship = "many-to-many") %>%
  group_by(focal, split, band) %>%
  summarise(grade_26 = mean(grade_26), supp_26 = mean(supp_26),
            interp_share = mean(interp_share), .groups = "drop")

## -- NE WALL B: league slate side == shipped slate, split x band -----------
gate_ne26_sec <- slate_band_sec_lg %>%
  filter(focal == "NE") %>%
  inner_join(cmp_slate_mz %>%
               select(split, band, g_canon = grade_26, s_canon = supp_26,
                      i_canon = interp_share),
             by = c("split", "band")) %>%
  mutate(dg = abs(grade_26 - g_canon), ds = abs(supp_26 - s_canon),
         di = abs(interp_share - i_canon))
stopifnot(nrow(gate_ne26_sec) == 2L * length(BANDS_SEC_LG))
cat("NE slate wall: max |d grade| =", signif(max(gate_ne26_sec$dg), 3),
    "| max |d supp| =", signif(max(gate_ne26_sec$ds), 3),
    "| max |d interp| =", signif(max(gate_ne26_sec$di), 3), "\n")
if (max(gate_ne26_sec$dg, gate_ne26_sec$ds, gate_ne26_sec$di) > 1e-8) {
  print(as.data.frame(gate_ne26_sec))
  stop("NE slate wall failed -- league slate does not reproduce canon")
}
cat("NE WALL B PASS: slate side reproduces cmp_slate_mz at 1e-8 (8 cells)\n")

## ... and unit grain (schedule-mean of opponent unit composites)
slate_unit_sec_lg <- opp26_sec %>%
  inner_join(team_unit_sec_lg, by = c("opp" = "team")) %>%
  group_by(focal, split) %>%
  summarise(unit_26 = mean(unit_26),
            interp_share = mean(interp_share), .groups = "drop")
stopifnot(nrow(slate_unit_sec_lg) == 64L)

## -- NE WALL C: unit grain == shipped cmp_unit_mz ---------------------------
gate_neu_sec <- slate_unit_sec_lg %>%
  filter(focal == "NE") %>%
  inner_join(cmp_unit_mz %>% select(split, u26_canon = unit_26,
                                    i_canon = interp_share),
             by = "split") %>%
  inner_join(faced_unit_sec_lg %>%
               filter(focal == "NE") %>%
               select(split, u25_lg = unit_25),
             by = "split") %>%
  inner_join(cmp_unit_mz %>% select(split, u25_canon = unit_25),
             by = "split") %>%
  mutate(d26 = abs(unit_26 - u26_canon), di = abs(interp_share - i_canon),
         d25 = abs(u25_lg - u25_canon))
stopifnot(nrow(gate_neu_sec) == 2L)
cat("NE unit wall: max |d unit26| =", signif(max(gate_neu_sec$d26), 3),
    "| max |d interp| =", signif(max(gate_neu_sec$di), 3),
    "| max |d unit25| =", signif(max(gate_neu_sec$d25), 3), "\n")
if (max(gate_neu_sec$d26, gate_neu_sec$di, gate_neu_sec$d25) > 1e-8) {
  print(as.data.frame(gate_neu_sec))
  stop("NE unit wall failed -- unit composite does not reproduce canon")
}
cat("NE WALL C PASS: unit composites reproduce cmp_unit_mz at 1e-8 (both splits)\n")

## ============================================================================
## 8. LEAGUE FRAME + PRINTS -- one row per team x split, NEVER recombined
##    d > 0 = opponents' 2026 coverage units are STRONGER than the 2025
##    units this passing game actually faced (incl. playoffs) = harder
## ============================================================================

league_cov <- faced_unit_sec_lg %>%
  select(focal, split, unit_25, fill_share_25) %>%
  inner_join(slate_unit_sec_lg, by = c("focal", "split")) %>%
  left_join(wk_diet_sec, by = "focal") %>%
  group_by(split) %>%
  mutate(d_grade  = unit_26 - unit_25,
         rank_d   = rank(-d_grade, ties.method = "min"),
         rank_u26 = rank(-unit_26, ties.method = "min")) %>%
  ungroup()

for (sp in c("man", "zone")) {
  cat("\n================ ", toupper(sp),
      " COVERAGE SoS DIFFERENTIAL LADDER (sorted by d_grade) ================\n", sep = "")
  cat("d > 0: 2026 opposing", sp, "coverage STRONGER than 2025 faced = HARDER\n\n")
  print(league_cov %>% filter(split == sp) %>% arrange(desc(d_grade)) %>%
...
summ_sec <- league_cov %>%
  group_by(split) %>%
  summarise(
    unit_25_mean = mean(unit_25), unit_25_sd = sd(unit_25),
    unit_26_mean = mean(unit_26), unit_26_sd = sd(unit_26),
    d_mean       = mean(d_grade), d_sd       = sd(d_grade),
    fill_mean    = mean(fill_share_25),
    interp_mean  = mean(interp_share),
    .groups = "drop")
print(as.data.frame(summ_sec))

ne_sec <- league_cov %>% filter(focal == "NE")
cat("\nNE:")
for (sp in c("man", "zone")) {
  r <- ne_sec %>% filter(split == sp)
  cat(sprintf("  %s d_grade=%+.4f (rank %d/32; unit_26 rank %d)",
              sp, r$d_grade, r$rank_d, r$rank_u26))
}
cat("\n")

cov_wide <- league_cov %>%
  select(focal, split, d_grade) %>%
  tidyr::pivot_wider(names_from = split, values_from = d_grade,
                     names_prefix = "d_")
cat(sprintf("cor(d_man, d_zone) across 32 teams = %.3f\n",
            cor(cov_wide$d_man, cov_wide$d_zone)))

cat("\n---- AFC East (ranks are league-wide within split, before filtering) ----\n")
print(league_cov %>% filter(focal %in% c("NE", "BUF", "MIA", "NYJ")) %>%
        arrange(split, desc(d_grade)) %>% as.data.frame(), row.names = FALSE)

cat("\n---- league mean sanity per split: redistribute, never inflate ----\n")
for (sp in c("man", "zone")) {
  v <- league_cov %>% filter(split == sp)
  cat(sprintf("  %s: mean(d_grade)=%+.5f  sd=%+.5f  (expect ~0 up to pool composition)\n",
              sp, mean(v$d_grade), sd(v$d_grade)))
}

cat("\nDone. Objects created (all *_sec_lg suffixed except league_cov):\n")
cat("  modal_band_sec_lg cov_pctl_sec_lg use_sec_lg band_map_sec_lg xw_sec_lg\n")
cat("  ent_sec_lg ros_sec_lg slot_sec_lg slot_med_sec_lg prior_law_sec_lg\n")
cat("  ledger_sec_lg rot_2026_sec_lg team_band_sec_lg team_unit_sec_lg\n")
cat("  band_wt_sec_lg faced_games_sec_lg faced_band_sec_lg\n")
cat("  faced_unit_sec_lg slate_band_sec_lg slate_unit_sec_lg league_cov\n")
