# ============================================================
# NE 2026 -- OPPONENT PASS BLOCK SoS -- LEAGUE MODE (Phase 1 league
# lens, PASS-PRO HALF). Written 2026-08-18.
#
...
# 32 -- the two schedules barely correlate. The run-block half
# (gap + zone) lives in league_opp_run_block_schedule.
#
# WHAT IT IS: the league engine pointed at opponent PASS PRO (tps
# lens only -- canon dropped pb; tps_press stays out per the
# 2026-08-08 ruling: pressure rides Fig 1 + tables only). Per focal:
#   v26     = mean of the focal's 17 2026 opponents' pass-pro unit
#             values (each opponent = mean of its five slot values).
#   faced25 = the 2025 pass-pro starters the focal's FRONT actually
#             faced (canon construction: one blocker per week x team
#             x slot by pass-block snaps), fill-law priced.
#   d       = v26 - faced25. d > 0 = the 2026 slate's pass-pro lines
...
#       qualifying season pctl (13 teams priced < 5 slots in tps;
#       KC's gap value came off TWO slots, MIA tps off THREE, and
#       prior_share reported 0 while it happened). Every slot now
#       prices, the rescue tier is counted in prior_share, and every
#       rescued slot is printed by name. n_scored == 5 is now a hard
#       wall, not a receipt.
...
league_sys_pb <- league_one_lens("tps")

# ------------------------------------------------------------
# D. THE LEAGUE PASS-PRO DIFFERENTIAL -- one lens, one frame
# ------------------------------------------------------------

league_wide_pb <- league_sys_pb %>%
  mutate(rank_tps = rank(-d)) %>%
  arrange(desc(d))

cat("\n--- THE OPPONENT PASS-PRO DIFFERENTIAL, sorted by d (tps lens) ---\n")
print(league_wide_pb %>%
        mutate(across(where(is.numeric), ~ round(.x, 3))), n = 32)

summ_pb <- league_sys_pb %>%
...
cat("\n--- league summary (tps lens) ---\n")
print(summ_pb)

cat("\nAFC East, pass-pro slate (rank 1 = schedule gets hardest):\n")
print(league_wide_pb %>% filter(focal %in% c("BUF", "MIA", "NE", "NYJ")) %>%
        select(focal, v26, faced25, d, rank_tps) %>%
...
if (exists("ne_2025_opp_ol_games") && exists("opp_ol_2026_final") &&
  ...
  # R1 -- FACED MACHINE CHECK: canon's NE slate_2025 per-slot tps
  ...
  mine_s25 <- all_pass_block_summary %>%
    ...
    left_join(ol_season_pctl %>%
              ...
              by = c("player_id", "det_position")) %>%
    group_by(det_position) %>%
    summarise(mine_tps = mean(tps_grade, na.rm = TRUE), .groups = "drop")
  chk_s25 <- canon_s25 %>% full_join(mine_s25, by = "det_position") %>%
    mutate(ok_tps = abs(canon_tps - mine_tps) < 1e-8)
  cat("\n--- R1: NE faced-2025 per-slot TPS, canon vs this file (pre-fill) ---\n")
  print(chk_s25 %>% mutate(across(where(is.numeric), ~ round(.x, 4))))
  stopifnot(all(chk_s25$ok_tps, na.rm = TRUE))
  
  # R2 -- THE FILL-LAW MOVE for NE (BEFORE = earned-only mean, the
  #   pre-fill convention; AFTER = the fill-law value).
  pre_tps <- all_pass_block_summary %>%
    filter(season == 2025, in_season(week), def_ssn == "NE2025",
           det_position %in% ol_pos_levels) %>%
    group_by(week, team_name, det_position) %>%
    slice_max(snap_counts_pass_block, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    left_join(ol_season_pctl %>%
                select(player_id, det_position, F25 = tps_grade),
              by = c("player_id", "det_position")) %>%
    summarise(pre = mean(F25, na.rm = TRUE)) %>% pull(pre)
  ne_row <- league_wide_pb %>% filter(focal == "NE")
  cat("\n--- R2: NE faced25 before/after the fill law (tps) ---\n")
  cat(sprintf("  tps: before %.4f | after %.4f | fill_share %.3f\n",
              pre_tps, ne_row$faced25, ne_row$fill_share))
  ...
  # R4 -- NE v26 under both memberships (the membership-swap
  #   magnitude on the one schedule canon certifies). NOT a wall.
  canon_s26 <- tibble(team_name = sched_2026) %>%
    left_join(opp_ol_2026_final %>%
                select(team_name, det_position, tps_f),
              by = "team_name", relationship = "many-to-many") %>%
    summarise(canon_tps = mean(tps_f, na.rm = TRUE))
  cat("\n--- R4: NE v26 tps, tribble membership (canon) vs this engine ---\n")
  cat(sprintf("  tps: canon %.4f | auto %.4f\n", canon_s26$canon_tps,
              ne_row$v26))
} else {
...
legend_pb <- c(
  "",
  "============================================================",
  "HOW TO READ WHAT YOU JUST BUILT -- OPPONENT PASS BLOCK, LEAGUE",
  "============================================================",
  "One row per focal team. Pass pro only (tps lens; canon dropped",
  "pb, and tps_press stays in Fig 1 + tables per the 2026-08-08",
  "ruling). Run blocking lives in league_opp_run_block_schedule --",
  "the two are different markets (cor(d_tps, d_gap) = 0.24).",
  "",
  "  v26        mean of the focal's 17 2026 opponents' pass-pro unit",
  "             values; each opponent = mean of its five slot values.",
  "             THE 2026 SLATE.",
  "  faced25    the 2025 pass-pro starters the focal's FRONT actually",
  "             faced: one blocker per week x team x slot by",
  "             pass-block snaps (canon construction), priced with",
  "             the earned season pctl or a prior (fill law).",
  "  d          v26 - faced25 = the YoY schedule differential.",
  "             d > 0: the 2026 slate's pass-pro lines are BETTER",
  "             than the lines this front beat on in 2025.",
  "  prior_share share of the focal's opponent slot weight riding",
  "             priors/phantoms/rescues (v26 honesty).",
  "  fill_share  share of faced-2025 cells priced at priors.",
  "  rank_tps    rank(-d) across 32; 1 = schedule gets hardest.",
  "",
  ...
  "(canon v26 0.5210 vs faced 0.4750). The RANK is the signal; the",
  "sign is structural.",
  "",
...
# ------------------------------------------------------------
# save.image("~/league_opp_pass_block_workspace.RData")
# system('aws s3 cp ~/league_opp_pass_block_workspace.RData s3://nfl-pff-data-lucas/workspaces/')
