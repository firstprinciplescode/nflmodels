...
prepare_task <- function(df, task_id) {
  df$pass_attempt <- as.factor(df$pass_attempt)
...
train_final_model <- function(df, best_params, target_col = "pass_attempt") {
...
# ============================================================================
# DATA PREPARATION (part_nfl - WITH PARTICIPATION)
# ============================================================================

xgboost_part_base <- part_nfl %>% 
  filter(two_point_attempt == 0 & fumble == 0 & qb_kneel == 0 & 
           !is.na(defenders_in_box) & play_type %in% c("run", "pass")) %>%
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(pass_attempt, yardline_100, defenders_in_box, half_seconds_remaining, 
         shotgun, no_huddle, posteam_timeouts_remaining, defteam_timeouts_remaining, 
         score_differential, mod_ydstogo, n_rb:n_st_def, i_form_ind:wildcat_ind, 
         d43_ind:punt_ind_def, temp, wind, rain_ind, snow_ind)

cat("xpass (with participation) rows:", nrow(xgboost_part_base), "\n\n")
...
task_part_xpass <- prepare_task(xgboost_part_base, "part_xpass_weather")
tuning_part_xpass <- tune_xgb_model(task_part_xpass, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_part_xpass$best_params)
cat("\nBest CV LogLoss:", round(tuning_part_xpass$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_part_base, tuning_part_xpass$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "xpass - Participation (with weather)"
)
...
artifacts <- list(
  ...
  best_params = tuning_part_xpass$best_params,
