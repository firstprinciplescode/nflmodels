...
prepare_task <- function(df, task_id, target_col = "yards_gained") {
  TaskRegr$new(id = task_id, backend = df, target = target_col)
}
...
tune_xgb_model <- function(task, n_evals = 125) {
  ...
  instance <- TuningInstanceBatchSingleCrit$new(
    ...
    measure = measure, search_space = search_space,
  ...
  list(best_params = instance$result_learner_param_vals,
...
evaluate_model <- function(model, dtest, y_test, model_name = "Model") {
  ...
  resid_df <- data.frame(Predicted = preds, Actual = y_test, Residual = y_test - preds)
  
  cat("Residual Summary:\n")
  print(summary(resid_df$Residual))
  
  list(rmse = rmse_score, mae = mae_score, r2 = r2_score, residuals = resid_df)
}
...
xgboost_pbp_base <- pbp_base %>%
  ...
  select(yards_gained, yardline_100, season_type, half_seconds_remaining, down,
...
cat("Scramble YPC rows:", nrow(xgboost_pbp_base), "\n\n")
...
task_scramble_ypc   <- prepare_task(xgboost_pbp_base, "pbp_scramble_ypc_weather")
tuning_scramble_ypc <- tune_xgb_model(task_scramble_ypc, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_scramble_ypc$best_params)
cat("\nBest CV RMSE:", round(tuning_scramble_ypc$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_scramble_ypc$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "Scramble YPC (Weather)"
)

# Predicted vs Actual plot
ggplot(eval_results$residuals, aes(x = Actual, y = Predicted)) +
  geom_point(alpha = 0.15, size = 0.8) +
  geom_abline(slope = 1, intercept = 0, color = "red", linewidth = 1) +
  labs(title = "PBP Scramble YPC: Predicted vs Actual",
       x = "Actual Yards", y = "Predicted Yards") +
  theme_minimal()
...
model_name <- "xgb_pbp_scramble_ypc"

model_path <- paste0(getwd(), '/', model_name, '.model')
xgb.save(final_model$model, model_path)
put_object(file = model_path, object = paste0("models/", model_name, ".model"), bucket = "nfl-pff-data-lucas")
cat("Model saved to S3!\n")

artifacts <- list(feature_names = final_model$feature_names, feature_importance = importance,
                  best_params = tuning_scramble_ypc$best_params, trained_date = Sys.time())
artifacts_path <- paste0(getwd(), '/', model_name, '_artifacts.rds')
saveRDS(artifacts, artifacts_path)
put_object(file = artifacts_path, object = paste0("models/", model_name, "_artifacts.rds"), bucket = "nfl-pff-data-lucas")
