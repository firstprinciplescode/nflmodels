...
part_nfl %>%
  filter(qb_scramble == 1 & is.na(air_yards)) %>%
  select(middle_gap_ind, guard_gap_ind, tackle_gap_ind, end_gap_ind)
...
xgboost_part_base <- part_nfl %>% 
  filter(two_point_attempt == 0 & fumble == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & qb_scramble == 1 & !is.na(pressure_ind) & is.na(time_to_throw) & is.na(air_yards)) %>% 
  select(pressure_ind, qb_hit, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind)

xgboost_part_base_weather <- part_nfl %>% 
  filter(two_point_attempt == 0 & fumble == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & qb_scramble == 1 & !is.na(pressure_ind) & is.na(time_to_throw) & is.na(air_yards) & !is.na(temp) & !is.na(wind)) %>% 
  select(pressure_ind, qb_hit, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind, temp, wind)
...
# Save locally
model_path <- paste0(getwd(), '/xgb_part_pressure_after_ttt_scramble_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_pressure_after_ttt_scramble_no_temp.model",
...
# Save locally
model_path <- paste0(getwd(), '/xgb_part_pressure_after_ttt_scramble_temp.model')
xgb.save(final_model_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_pressure_after_ttt_scramble_temp.model",
  bucket = "nfl-pff-data-lucas"
)
