...
xgboost_pbp_base <- pbp_base %>% 
  filter(play_type == "pass" & qb_spike == 0 & qb_kneel == 0 & sack == 0 & pass_attempt == 1 & qb_scramble == 0 & !is.na(air_yards)) %>% 
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(td_side, yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, qb_dropback, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, temp, wind, rain_ind, snow_ind)
...
task_pass_td <- prepare_task(xgboost_pbp_base, "pass_td")
tuning_pass_td <- tune_xgb_model(task_pass_td, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_pass_td$best_params)
cat("\nBest CV LogLoss:", round(tuning_pass_td$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_pass_td$best_params)
...
model_path <- paste0(getwd(), '/xgb_pbp_pass_td.model')
xgb.save(final_model$model, model_path)

put_object(
  file = model_path,
  object = "models/xgb_pbp_pass_td.model",
...
artifacts <- list(
  ...
  best_params = tuning_pass_td$best_params,
  trained_date = Sys.time()
)

artifacts_path <- paste0(getwd(), '/xgb_pbp_pass_td_artifacts.rds')
saveRDS(artifacts, artifacts_path)

put_object(
  file = artifacts_path,
  object = "models/xgb_pbp_pass_td_artifacts.rds",
  bucket = "nfl-pff-data-lucas"
)
