...
xgboost_pbp_base <- pbp_base %>% 
  filter((pass_attempt == 1 | rush_attempt == 1) & play_type %in% c("run", "pass") & 
           two_point_attempt == 0 & qb_kneel == 0 & qb_spike == 0) %>%
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(pass_attempt, yardline_100, half_seconds_remaining, posteam_timeouts_remaining, 
         defteam_timeouts_remaining, mod_ydstogo, posteam_ind, down_one_ind, down_two_ind, 
         down_three_ind, down, shotgun, no_huddle, score_differential, 
         temp, wind, rain_ind, snow_ind)

cat("xpass (no participation) rows:", nrow(xgboost_pbp_base), "\n\n")
...
task_xpass <- prepare_task(xgboost_pbp_base, "pbp_xpass_weather")
tuning_xpass <- tune_xgb_model(task_xpass, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_xpass$best_params)
cat("\nBest CV LogLoss:", round(tuning_xpass$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_xpass$best_params)
...
artifacts <- list(
  ...
  best_params = tuning_xpass$best_params,
