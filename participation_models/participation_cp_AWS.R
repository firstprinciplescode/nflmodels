...
prepare_task <- function(df, task_id) {
  ...
  TaskClassif$new(id = task_id, backend = df, target = "complete_pass")
}
...
evaluate_model <- function(model, dtest, y_test, model_name = "Model") {
  ...
  calibration <- data.frame(Preds = preds, Vals = y_test,
  ...
  cat("AUC:", round(auc_score, 6), "\n")
  cat("Accuracy:", round(cm$overall['Accuracy'], 4), "\n\n")
  cat("Calibration:\n")
  print(calibration)
  
  list(auc = auc_score, logloss = logloss_score, confusion_matrix = cm, calibration = calibration)
}
...
xgboost_part_base <- part_nfl %>%
  filter(play_type == "pass" & sack == 0 & qb_spike == 0 & qb_kneel == 0 &
           two_point_attempt == 0 & pass_attempt == 1 & rush_attempt == 0 & !is.na(air_yards)) %>%
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(complete_pass, yardline_100, season_type, half_seconds_remaining, down,
         down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers,
         mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind,
         n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db,
         cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind,
         ...
         air_yards, pressure_ind, temp, wind, rain_ind, snow_ind)

cat("Completion probability (participation) rows:", nrow(xgboost_part_base), "\n\n")
...
task_part_cp   <- prepare_task(xgboost_part_base, "part_cp_weather")
tuning_part_cp <- tune_xgb_model(task_part_cp, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_part_cp$best_params)
cat("\nBest CV LogLoss:", round(tuning_part_cp$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_part_base, tuning_part_cp$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "Completion Probability (with weather)"
)
...
artifacts <- list(
  ...
  best_params = tuning_part_cp$best_params,
