...
xgboost_pbp_base <- pbp_base %>%
  filter(play_type == "run" & qb_scramble == 0 & qb_spike == 0 & qb_kneel == 0 &
           two_point_attempt == 0 & rush_attempt == 1 & pass_attempt == 0) %>%
...
cat("YPC (no participation) rows:", nrow(xgboost_pbp_base), "\n\n")
...
task_pbp_ypc   <- prepare_task(xgboost_pbp_base, "pbp_ypc_weather")
tuning_pbp_ypc <- tune_xgb_model(task_pbp_ypc, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_pbp_ypc$best_params)
cat("\nBest CV RMSE:", round(tuning_pbp_ypc$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_pbp_ypc$best_params)
...
ggplot(eval_results$residuals, aes(x = Actual, y = Predicted)) +
  ...
  labs(title = "PBP YPC: Predicted vs Actual",
...
artifacts <- list(
  ...
  best_params = tuning_pbp_ypc$best_params,
