...
xgboost_pbp_base <- pbp_base %>% 
  ...
  select(yards_gained, yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, temp, wind, rain_ind, snow_ind)
...
task_ypa <- prepare_task(xgboost_pbp_base, "ypa")
tuning_ypa <- tune_xgb_model(task_ypa, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_ypa$best_params)
cat("\nBest CV RMSE:", round(tuning_ypa$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_ypa$best_params)
...
model_path <- paste0(getwd(), '/xgb_pbp_ypa.model')
xgb.save(final_model$model, model_path)

put_object(
  file = model_path,
  object = "models/xgb_pbp_ypa.model",
...
artifacts <- list(
  ...
  best_params = tuning_ypa$best_params,
  trained_date = Sys.time()
)

artifacts_path <- paste0(getwd(), '/xgb_pbp_ypa_artifacts.rds')
saveRDS(artifacts, artifacts_path)

put_object(
  file = artifacts_path,
  object = "models/xgb_pbp_ypa_artifacts.rds",
  bucket = "nfl-pff-data-lucas"
)
