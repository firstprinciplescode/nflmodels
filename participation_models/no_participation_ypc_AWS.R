...
xgboost_pbp_base <- pbp_base %>% 
  ...
  select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, end_ind, guard_ind, tackle_ind)

xgboost_pbp_base_weather <- pbp_base %>% 
  filter(play_type == "run" & qb_scramble == 0 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & rush_attempt == 1 & pass_attempt == 0 & !is.na(temp) & !is.na(wind)) %>% 
  select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, end_ind, guard_ind, tackle_ind, temp, wind)
...
task_before_xtd_no_temp <- prepare_task(xgboost_pbp_base, "ypc_no_temp")
...
task_before_xtd_temp <- prepare_task(xgboost_pbp_base_weather, "ypc_no_temp")
...
X_test_no_temp <- test_common %>% 
  select(yardline_100, season_type, half_seconds_remaining, down, down_one_ind, 
...
# Save locally
model_path <- paste0(getwd(), '/xgb_pbp_ypc_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_pbp_ypc_no_temp.model",
