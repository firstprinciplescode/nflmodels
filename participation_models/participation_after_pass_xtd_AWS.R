...
xgboost_part_base <- part_nfl %>%
  filter((play_type == "pass") & sack == 0 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & pass_attempt == 1 & rush_attempt == 0 & !is.na(air_yards)) %>% 
  select(td_side, qb_hit, time_to_throw, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind, i_form_ind:zone_ind, d43_ind:punt_ind_def, air_yards)

xgboost_part_base_weather <- part_nfl %>%
  filter((play_type == "pass") & sack == 0 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & pass_attempt == 1 & rush_attempt == 0 & !is.na(air_yards) & !is.na(temp) & !is.na(wind)) %>% 
  select(td_side, qb_hit, time_to_throw, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind, i_form_ind:zone_ind, d43_ind:punt_ind_def, air_yards, temp, wind)
...
task_before_xtd_no_temp <- prepare_task(xgboost_part_base, "part_after_pass_xtd_no_temp")
tuning_before_xtd_no_temp <- tune_xgb_model(task_before_xtd_no_temp, n_evals = 125)

tuning_before_xtd_no_temp$best_params
tuning_before_xtd_no_temp$best_score
...
# 1. Save the model
# Save locally
model_path <- paste0(getwd(), '/xgb_part_after_pass_xtd_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_after_pass_xtd_no_temp.model",
...
model_name <- "xgb_part_after_pass_xtd_no_temp"
