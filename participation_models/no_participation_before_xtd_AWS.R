...
xgboost_pbp_base <- pbp_base %>% 
  filter(two_point_attempt == 0 & qb_kneel == 0 & qb_spike == 0 & play_type %in% c("pass", "run")) %>% 
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(td_side, yardline_100, half_seconds_remaining, mod_ydstogo, posteam_ind, down, 
         shotgun, no_huddle, score_differential, posteam_timeouts_remaining, 
         defteam_timeouts_remaining, temp, wind, rain_ind, snow_ind)
...
task_before_xtd <- prepare_task(xgboost_pbp_base, "before_xtd_weather")
tuning_before_xtd <- tune_xgb_model(task_before_xtd, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_before_xtd$best_params)
cat("\nBest CV LogLoss:", round(tuning_before_xtd$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_before_xtd$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "Before XTD (with weather)"
)
...
model_path <- paste0(getwd(), '/xgb_pbp_before_td.model')
xgb.save(final_model$model, model_path)

put_object(
  file = model_path,
  object = "models/xgb_pbp_before_td.model",
...
artifacts <- list(
  ...
  best_params = tuning_before_xtd$best_params,
  trained_date = Sys.time()
)

artifacts_path <- paste0(getwd(), '/xgb_pbp_before_td_artifacts.rds')
saveRDS(artifacts, artifacts_path)

put_object(
  file = artifacts_path,
  object = "models/xgb_pbp_before_td_artifacts.rds",
  bucket = "nfl-pff-data-lucas"
)
