...
task_yac <- prepare_task(xgboost_pbp_base, "yds_after_catch")
tuning_yac <- tune_xgb_model(task_yac, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_yac$best_params)
cat("\nBest CV RMSE:", round(tuning_yac$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_yac$best_params)
...
model_path <- paste0(getwd(), '/xgb_pbp_yds_after_catch.model')
xgb.save(final_model$model, model_path)

put_object(
  file = model_path,
  object = "models/xgb_pbp_yds_after_catch.model",
...
artifacts <- list(
  ...
  best_params = tuning_yac$best_params,
  trained_date = Sys.time()
)

artifacts_path <- paste0(getwd(), '/xgb_pbp_yds_after_catch_artifacts.rds')
saveRDS(artifacts, artifacts_path)

put_object(
  file = artifacts_path,
  object = "models/xgb_pbp_yds_after_catch_artifacts.rds",
  bucket = "nfl-pff-data-lucas"
)
