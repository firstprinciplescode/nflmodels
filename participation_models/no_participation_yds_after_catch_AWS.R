...
xgboost_pbp_base <- pbp_base %>% 
  filter(play_type == "pass" & complete_pass == 1 & qb_spike == 0 & qb_kneel == 0 & sack == 0 & pass_attempt == 1 & qb_scramble == 0 & !is.na(air_yards) & !is.na(yards_after_catch)) %>% 
  select(yards_after_catch, yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, qb_dropback, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, air_yards, down_one_ind, down_two_ind, down_three_ind)

xgboost_pbp_base_weather <- pbp_base %>% 
  filter(play_type == "pass" & complete_pass == 1 & qb_spike == 0 & qb_kneel == 0 & sack == 0 & pass_attempt == 1 & qb_scramble == 0 & !is.na(air_yards) & !is.na(yards_after_catch)) %>% 
  select(yards_after_catch, yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, qb_dropback, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, air_yards, down_one_ind, down_two_ind, down_three_ind, temp, wind)
...
task_before_xtd_no_temp <- prepare_task(xgboost_pbp_base, "yds_after_catch_no_temp")
...
task_before_xtd_temp <- prepare_task(xgboost_pbp_base_weather, "yds_after_catch_temp")
...
split_common <- sample.split(Y = xgboost_pbp_base_weather$yards_after_catch, SplitRatio = 0.8)
...
X_test_no_temp <- test_common %>% 
  select(yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, air_yards, down_one_ind, down_two_ind, down_three_ind) %>% 
  as.data.frame()

# Temp model - include temp/wind
X_test_temp <- test_common %>% 
  select(yardline_100, half_seconds_remaining, shotgun, no_huddle, mod_ydstogo, air_yards, pass_length, middle_ind, outside_ind, score_differential, qb_hit, roof, surface, posteam_timeouts_remaining, defteam_timeouts_remaining, air_yards, down_one_ind, down_two_ind, down_three_ind, temp, wind) %>% 
  as.data.frame()
...
# Save locally
model_path <- paste0(getwd(), '/xgb_pbp_yds_after_catch_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_pbp_yds_after_catch_no_temp.model",
...
# Save locally
model_path <- paste0(getwd(), '/xgb_pbp_yds_after_catch_temp.model')
xgb.save(final_model_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_pbp_yds_after_catch_temp.model",
  bucket = "nfl-pff-data-lucas"
)
