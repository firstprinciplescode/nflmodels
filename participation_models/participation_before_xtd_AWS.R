...
# ============================================================================
# DATA PREPARATION (part_nfl - EXPECTED TD)
# ============================================================================

xgboost_part_base <- part_nfl %>% 
  filter(two_point_attempt == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & 
           play_type %in% c("run", "pass")) %>%
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(td_side, yardline_100, defenders_in_box, half_seconds_remaining, shotgun, 
         ...
         cover3_ind:zone_ind, d43_ind:punt_ind_def, temp, wind, rain_ind, snow_ind)

cat("Expected TD rows:", nrow(xgboost_part_base), "\n\n")
...
task_part_xtd <- prepare_task(xgboost_part_base, "part_xtd_weather")
tuning_part_xtd <- tune_xgb_model(task_part_xtd, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_part_xtd$best_params)
cat("\nBest CV LogLoss:", round(tuning_part_xtd$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_part_base, tuning_part_xtd$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "Expected TD - With Participation (with weather)"
)
...
artifacts <- list(
  ...
  best_params = tuning_part_xtd$best_params,
