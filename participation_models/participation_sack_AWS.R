...
train_final_model <- function(df, best_params, target_col = "sack") {
...
xgboost_part_base <- part_nfl %>% 
  filter(two_point_attempt == 0 & fumble == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & play_type %in% c("pass") & !is.na(sack)) %>% 
  select(sack, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, i_form_ind:st_ind, d43_ind:punt_ind_def, rain_ind, snow_ind)
...
task_before_xtd_no_temp <- prepare_task(xgboost_part_base, "sack_no_temp")
...
# 1. Save the model
# Save locally
model_path <- paste0(getwd(), '/xgb_part_sack_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_sack_no_temp.model",
...
artifacts_path <- paste0(getwd(), '/xgb_part_sack_no_temp_artifacts.rds')
saveRDS(artifacts, artifacts_path)

# Upload to S3
put_object(
  file = artifacts_path,
  object = "models/xgb_part_sack_no_temp_artifacts.rds",
  bucket = "nfl-pff-data-lucas"
)
