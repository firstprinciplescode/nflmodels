...
prepare_task <- function(df, task_id, target_col = "sack") {
...
# ============================================================================
# DATA PREPARATION (pbp_base - SACK PROBABILITY)
# ============================================================================

xgboost_pbp_base <- pbp_base %>% 
  filter(play_type == "pass" & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0) %>%
  mutate(
    wind = ifelse(is.na(wind), 0, wind),
    temp = ifelse(is.na(temp), 70, temp)
  ) %>%
  select(sack, yardline_100, season_type, half_seconds_remaining, down, 
...
cat("Sack probability rows:", nrow(xgboost_pbp_base), "\n\n")
...
task_pbp_sack <- prepare_task(xgboost_pbp_base, "pbp_sack_weather")
tuning_pbp_sack <- tune_xgb_model(task_pbp_sack, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_pbp_sack$best_params)
cat("\nBest CV LogLoss:", round(tuning_pbp_sack$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_pbp_base, tuning_pbp_sack$best_params)

eval_results <- evaluate_model(
  final_model$model,
  final_model$dtest,
  final_model$y_test,
  "Sack Probability (with weather)"
)
...
artifacts <- list(
  ...
  best_params = tuning_pbp_sack$best_params,
