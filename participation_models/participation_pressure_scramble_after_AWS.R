...
xgboost_part_base <- part_nfl %>% 
  filter(two_point_attempt == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & qb_scramble == 1 & rush_attempt == 1 & !is.na(pressure_ind) & is.na(time_to_throw) & is.na(air_yards)) %>% 
  select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, i_form_ind:st_ind, d43_ind:punt_ind_def, guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind, outside_run_ind, middle_run_ind)

xgboost_part_base_weather <- part_nfl %>% 
  filter(two_point_attempt == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & qb_scramble == 1 & rush_attempt == 1 & !is.na(pressure_ind) & is.na(time_to_throw) & is.na(air_yards) & !is.na(temp) & !is.na(wind)) %>% 
  select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, i_form_ind:st_ind, d43_ind:punt_ind_def, guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind, outside_run_ind, middle_run_ind, temp, wind)
...
task_before_xtd_no_temp <- prepare_task(xgboost_part_base, "pressure_scramble_no_temp")
...
task_before_xtd_temp <- prepare_task(xgboost_part_base_weather, "pressure_scramble_temp")
...
X_test_no_temp <- test_common %>% 
  select(yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def, i_form_ind:st_ind, d43_ind:punt_ind_def, guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind, outside_run_ind, middle_run_ind)
...
# 1. Save the model
# Save locally
model_path <- paste0(getwd(), '/xgb_part_pressure_scramble_no_temp.model')
xgb.save(final_model_no_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_pressure_scramble_no_temp.model",
...
model_name <- "xgb_part_pressure_scramble_no_temp"
...
model_path <- paste0(getwd(), '/xgb_part_pressure_scramble_temp.model')
xgb.save(final_model_temp$model, model_path)

# Upload to S3
put_object(
  file = model_path,
  object = "models/xgb_part_pressure_scramble_temp.model",
  bucket = "nfl-pff-data-lucas"
)
...
model_name <- "xgb_part_pressure_scramble_temp"

# Save artifacts
artifacts <- list(
  feature_names = final_model_temp$feature_names,
  feature_importance = importance_temp,
  trained_date = Sys.time()
)
