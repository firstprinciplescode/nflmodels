...
xgboost_part_nfl <- part_nfl %>%
  filter(qb_scramble == 0 & rush_attempt == 1 & qb_spike == 0 & qb_kneel == 0 &
           two_point_attempt == 0 & pass_attempt == 0) %>%
  select(yards_gained, yardline_100, season_type, half_seconds_remaining, down,
         down_one_ind, down_two_ind, down_three_ind, defenders_in_box, mod_ydstogo,
         shotgun, no_huddle, score_differential, surface, posteam_ind,
         n_ol, n_te, n_rb, n_wr, n_st, n_dl, n_lb, n_db, n_st_def,
         i_form_ind:st_ind, d43_ind:punt_ind_def,
         guard_gap_ind, tackle_gap_ind, end_gap_ind, middle_gap_ind,
         temp, wind, rain_ind, snow_ind)

cat("YPC (with participation) rows:", nrow(xgboost_part_nfl), "\n\n")
...
task_part_ypc   <- prepare_task(xgboost_part_nfl, "part_ypc_weather")
tuning_part_ypc <- tune_xgb_model(task_part_ypc, n_evals = 125)

cat("\nBest hyperparameters found:\n")
print(tuning_part_ypc$best_params)
cat("\nBest CV RMSE:", round(tuning_part_ypc$best_score, 4), "\n\n")

final_model <- train_final_model(xgboost_part_nfl, tuning_part_ypc$best_params)
...
ggplot(eval_results$residuals, aes(x = Actual, y = Predicted)) +
  ...
  labs(title = "YPC: Predicted vs Actual",
...
artifacts <- list(
  ...
  best_params = tuning_part_ypc$best_params,
