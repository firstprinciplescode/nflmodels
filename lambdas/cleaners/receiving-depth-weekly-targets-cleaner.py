...
def lambda_handler(event, context):
    ...
    if isinstance(seasons, str) and seasons.lower() == 'all':
        seasons = list(range(2015, 2030))  # Adjust range as needed
    
    results = []
    
    for season in seasons:
        s3_key = f'data/receiving_depth_weekly_targets/season={season}/data.parquet'
        ...
        try:
            ...
            for col in df.columns:
                if df[col].dtype in ['int64', 'Int64', 'int32', 'Int32', 'bigint', 'BIGINT']:
