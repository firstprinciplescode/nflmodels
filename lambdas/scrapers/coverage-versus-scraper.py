...
def scrape_receiving_coverage_versus(game_id, season, week, auth):
    """Scrape receiving coverage versus (matchup-level) data for ONE game"""
    url = f'https://api.pff.com/v1/facet/receiving/coverage?game_id={game_id}'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'restricted' in data and data['restricted']:
                raise Exception(f"RESTRICTED ACCESS - auth expired. Missing fields: {', '.join(str(f) for f in data['restricted'][:5])}...")
            ...
            if 'restricted' in recv_cov and recv_cov['restricted']:
                raise Exception(f"RESTRICTED ACCESS - auth expired. Missing fields: {', '.join(str(f) for f in recv_cov['restricted'][:5])}...")
