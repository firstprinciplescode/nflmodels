...
def scrape_pass_rush_summary(season, week, cookies):
    url = f'https://premium.pff.com/api/v1/facet/defense/pass_rush?league=nfl&season={season}&week={week}'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            pass_rush_data = data.get('pass_rush_summary', [])
            
            if not pass_rush_data:
                return pd.DataFrame()
            
            df = pd.DataFrame(pass_rush_data)
            df['week'] = week
            df['season'] = season
...
def save_to_s3(df, season):
    type_mapping = {
        ...
        'hits': 'float64',
        'hurries': 'float64',
        'total_pressures': 'float64',
        'batted_passes': 'float64',
        'penalties': 'float64',
        'declined_penalties': 'float64',
        'pass_rush_wins': 'float64',
        'pass_rush_opp': 'float64',
        'snap_counts_pass_rush': 'float64',
        'snap_counts_pass_play': 'float64',
        'true_pass_set_sacks': 'float64',
        'true_pass_set_hits': 'float64',
        'true_pass_set_hurries': 'float64',
        'true_pass_set_total_pressures': 'float64',
        'true_pass_set_batted_passes': 'float64',
        'true_pass_set_pass_rush_wins': 'float64',
        'true_pass_set_pass_rush_opp': 'float64',
        'true_pass_set_snap_counts_pass_rush': 'float64',
        'true_pass_set_snap_counts_pass_play': 'float64',
        'prp': 'float64',
        'pass_rush_win_rate': 'float64',
        'pass_rush_percent': 'float64',
        'grades_pass_rush_defense': 'float64',
        'true_pass_set_prp': 'float64',
        'true_pass_set_pass_rush_win_rate': 'float64',
        'true_pass_set_pass_rush_percent': 'float64',
        'true_pass_set_grades_pass_rush_defense': 'float64',
        'player': 'str',
        'position': 'str',
        'team': 'str',
        'team_name': 'str',
    ...
    s3_key = f'data/pass_rush_summary/season={season}/data.parquet'
...
def lambda_handler(event, context):
    try:
        ...
        print("=" * 60)
        print(f"PASS RUSH SUMMARY SCRAPER - Season {season}")
        print(f"Weeks: {weeks}")
        print("=" * 60)
        ...
        for idx, week in enumerate(weeks, 1):
            ...
            df = scrape_pass_rush_summary(season, week, cookies)
