...
def scrape_passing_depth_for_week(season, week, cookies):
    """
    Scrape passing depth data for one season/week combination
    Returns: DataFrame with passing depth data
    """
    url = f'https://premium.pff.com/api/v1/facet/passing/depth?league=nfl&season={season}&week={week}'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            depth_data = data.get('passing_depth', [])
            ...
            if depth_data:
                first_record = depth_data[0]
                critical_fields = ['center_short_grades_pass', 'deep_grades_pass', 'short_grades_pass']
            ...
            print(f"  ✓ {season} Week {week}: {len(depth_data)} records")
...
def scrape_all_passing_depth(cookies, season, weeks):
    """Scrape passing depth data for all weeks in a season"""
    all_data = []
    
    print(f"\nScraping {season} - {len(weeks)} weeks...")
    
    for week in weeks:
        df = scrape_passing_depth_for_week(season, week, cookies)
    ...
    if all_data:
        ...
        print(f"✓ Total: {len(combined)} passing depth records")
        return combined
    else:
        return pd.DataFrame()
...
def save_to_s3_by_season(df, season):
    """
    Save passing depth data to S3, partitioned by season
    APPENDS to existing file and deduplicates by player_id/week/season
    """
    
    # Cast columns based on patterns
    for col in df.columns:
        try:
            if col in ['player', 'position', 'team', 'team_name', 'jersey_number', 'scraped_at']:
                df[col] = df[col].astype('str')
            elif col in ['season', 'week', 'player_id', 'franchise_id', 'draft_season', 'eligible_season', 'player_game_count', 'dropbacks']:
                df[col] = df[col].astype('Int64')
            elif any(x in col for x in ['dropbacks', 'attempts', 'completions', 'yards', 'touchdowns', 'interceptions', 
                                         'sacks', 'scrambles', 'first_downs', 'drops', 'bats', 'big_time_throws',
                                         'turnover_worthy_plays', 'def_gen_pressures', 'hit_as_threw', 
                                         'spikes', 'thrown_aways', 'aimed_passes', 'passing_snaps']):
                if col not in ['season', 'week']:
                    df[col] = df[col].astype('Int64')
            elif col not in ['player', 'position', 'team', 'team_name', 'jersey_number', 'scraped_at']:
                df[col] = df[col].astype('float64')
        except Exception as e:
            print(f"Warning: Could not cast column {col}: {e}")
            pass
    
    s3_key = f"data/passing_depth/season={season}/data.parquet"
...
def lambda_handler(event, context):
    """
    Lambda handler for passing depth data scraper
    ...
    try:
        ...
        print("=" * 60)
        print(f"PASSING DEPTH SCRAPER - SEASON {season}")
        print(f"Weeks: {weeks}")
        print("=" * 60)
        
        cookies = get_cookies()
        
        df = scrape_all_passing_depth(cookies, season, weeks)
        
        if df.empty:
            return {
                'statusCode': 200,
                'body': json.dumps({'message': f'No passing depth data found for season {season}'})
            }
