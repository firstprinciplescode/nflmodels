...
def scrape_pass_rush_kpis(season, week, cookies):
    url = f'https://premium.pff.com/api/v1/facet/signature/defense/outside_pass_rush?league=nfl&season={season}&week={week}'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            prps_data = data.get('prps', [])
            
            if not prps_data:
                return pd.DataFrame()
            
            df = pd.DataFrame(prps_data)
            df['week'] = week
            df['season'] = season
...
def save_to_s3(df, season):
    ...
    s3_key = f'data/pass_rush_kpis/season={season}/data.parquet'
...
def lambda_handler(event, context):
    try:
        ...
        print("=" * 60)
        print(f"PASS RUSH KPIS SCRAPER - Season {season}")
        print(f"Weeks: {weeks}")
        print("=" * 60)
        ...
        for idx, week in enumerate(weeks, 1):
            ...
            df = scrape_pass_rush_kpis(season, week, cookies)
