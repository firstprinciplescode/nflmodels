import json
import boto3
import requests
import pandas as pd
from io import BytesIO
from datetime import datetime
import time

s3_client = boto3.client('s3')
secrets_client = boto3.client('secretsmanager')

BUCKET_NAME = 'nfl-pff-data-lucas'
SECRET_NAME = 'pff-api-cookies'

def get_cookies():
    response = secrets_client.get_secret_value(SecretId=SECRET_NAME)
    return json.loads(response['SecretString'])

def scrape_rushing_summary(season, week, cookies):
    url = f'https://premium.pff.com/api/v1/facet/rushing/summary?league=nfl&season={season}&week={week}'
    
    try:
        response = requests.get(url, cookies=cookies, timeout=10)
        
        if response.status_code == 200:
            data = response.json()
            
            if 'restricted' in data and data['restricted']:
                raise Exception(f"RESTRICTED ACCESS - cookies expired. Missing fields: {', '.join(data['restricted'][:5])}...")
            
            rushing_data = data.get('rushing_summary', [])
            
            if not rushing_data:
                return pd.DataFrame()
            
            df = pd.DataFrame(rushing_data)
            df['week'] = week
            df['season'] = season
            df['scraped_at'] = datetime.utcnow().isoformat()
            
            return df
        
        elif response.status_code == 401:
            raise Exception("401 Unauthorized - cookies expired!")
        else:
            return pd.DataFrame()
    
    except Exception as e:
        if "401" in str(e) or "Unauthorized" in str(e) or "RESTRICTED" in str(e):
            raise
        return pd.DataFrame()

def save_to_s3(df, season):
    type_mapping = {
        'player_id': 'Int64',
        'season': 'Int64',
        'week': 'Int64',
        'player_game_count': 'Int64',
        'franchise_id': 'Int64',
        'draft_season': 'Int64',
        'eligible_season': 'Int64',
        'attempts': 'Int64',
        'yards': 'Int64',
        'touchdowns': 'Int64',
        'first_downs': 'Int64',
        'fumbles': 'Int64',
        'longest': 'Int64',
        'total_touches': 'Int64',
        'receptions': 'Int64',
        'targets': 'Int64',
        'penalties': 'Int64',
        'declined_penalties': 'Int64',
        'avoided_tackles': 'Int64',
        'yards_after_contact': 'Int64',
        'explosive': 'Int64',
        'breakaway_attempts': 'Int64',
        'breakaway_yards': 'Int64',
        'designed_yards': 'Int64',
        'scramble_yards': 'Int64',
        'scrambles': 'Int64',
        'zone_attempts': 'Int64',
        'gap_attempts': 'Int64',
        'routes': 'Int64',
        'run_plays': 'Int64',
        'drops': 'Int64',
        'elu_rush_mtf': 'Int64',
        'elu_recv_mtf': 'Int64',
        'elu_yco': 'Int64',
        'rec_yards': 'Int64',
        'ypa': 'float64',
        'yprr': 'float64',
        'yco_attempt': 'float64',
        'breakaway_percent': 'float64',
        'elusive_rating': 'float64',
        'grades_offense': 'float64',
        'grades_run': 'float64',
        'grades_pass_route': 'float64',
        'grades_pass_block': 'float64',
        'grades_run_block': 'float64',
        'grades_hands_fumble': 'float64',
        'grades_offense_penalty': 'float64',
        'player': 'str',
        'position': 'str',
        'team': 'str',
        'team_name': 'str',
        'jersey_number': 'str',
        'scraped_at': 'str'
    }
    
    existing_columns = {k: v for k, v in type_mapping.items() if k in df.columns}
    df = df.astype(existing_columns)
    
    s3_key = f'data/rushing_summary/season={season}/data.parquet'
    
    # Try to load existing data
    try:
        response = s3_client.get_object(Bucket=BUCKET_NAME, Key=s3_key)
        existing_df = pd.read_parquet(BytesIO(response['Body'].read()))
        print(f"✓ Loaded existing file with {len(existing_df)} records")
        
        # Combine existing + new
        combined = pd.concat([existing_df, df], ignore_index=True)
        
        # Dedupe - keep last (newest) version
        before_dedup = len(combined)
        combined = combined.drop_duplicates(subset=['player_id', 'week', 'season'], keep='last')
        after_dedup = len(combined)
        print(f"✓ Deduped: {before_dedup} → {after_dedup} records")
        
        df = combined
        
    except s3_client.exceptions.NoSuchKey:
        print(f"No existing file - creating new")
    except Exception as e:
        print(f"Could not load existing file: {e} - creating new")
    
    # Save
    parquet_buffer = BytesIO()
    df.to_parquet(parquet_buffer, index=False, engine='pyarrow', compression='snappy')
    parquet_buffer.seek(0)
    
    s3_client.put_object(
        Bucket=BUCKET_NAME,
        Key=s3_key,
        Body=parquet_buffer.getvalue(),
        ContentType='application/octet-stream'
    )
    
    print(f"✓ Saved to s3://{BUCKET_NAME}/{s3_key}")
    return s3_key

def lambda_handler(event, context):
    """
    Event:
    {
        "season": 2024,
        "weeks": [1, 2, 3]
    }
    
    or
    
    {
        "season": 2024
    }
    """
    
    try:
        season = event.get('season')
        weeks = event.get('weeks')
        
        if not season:
            raise Exception("season parameter required")
        
        if not weeks:
            weeks = list(range(1, 19)) + [28, 29, 30, 32]
        
        print("=" * 60)
        print(f"RUSHING SUMMARY SCRAPER - Season {season}")
        print(f"Weeks: {weeks}")
        print("=" * 60)
        
        cookies = get_cookies()
        
        all_data = []
        total = len(weeks)
        
        for idx, week in enumerate(weeks, 1):
            print(f"Scraping week {week} ({idx}/{total})...")
            
            df = scrape_rushing_summary(season, week, cookies)
            
            if not df.empty:
                all_data.append(df)
            
            time.sleep(0.1)
        
        if not all_data:
            return {
                'statusCode': 200,
                'body': json.dumps({'message': 'No data found'})
            }
        
        combined = pd.concat(all_data, ignore_index=True)
        
        before_dedup = len(combined)
        combined = combined.drop_duplicates(subset=['player_id', 'week', 'season'], keep='first')
        after_dedup = len(combined)
        
        if before_dedup > after_dedup:
            print(f"✓ Removed {before_dedup - after_dedup} duplicates")
        
        s3_key = save_to_s3(combined, season)
        
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': 'Success',
                'season': season,
                'weeks': weeks,
                'records': len(combined),
                'unique_players': combined['player_id'].nunique(),
                's3_key': s3_key
            })
        }
        
    except Exception as e:
        print(f"ERROR: {e}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': str(e)})
        }