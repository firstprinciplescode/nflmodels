...
def scrape_run_defense_summary(season, week, cookies):
    url = f'https://premium.pff.com/api/v1/facet/defense/run?league=nfl&season={season}&week={week}'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            run_defense_data = data.get('run_defense_summary', [])
            
            if not run_defense_data:
                return pd.DataFrame()
            
            df = pd.DataFrame(run_defense_data)
            df['week'] = week
            df['season'] = season
...
def save_to_s3(df, season):
    type_mapping = {
        ...
        'run_stop_opp': 'Int64',
        'snap_counts_run': 'Int64',
        'avg_depth_of_tackle': 'float64',
    ...
    s3_key = f'data/run_defense_summary/season={season}/data.parquet'
...
def lambda_handler(event, context):
    try:
        ...
        print("=" * 60)
        print(f"RUN DEFENSE SUMMARY SCRAPER - Season {season}")
        print(f"Weeks: {weeks}")
        print("=" * 60)
        ...
        for idx, week in enumerate(weeks, 1):
            ...
            df = scrape_run_defense_summary(season, week, cookies)
