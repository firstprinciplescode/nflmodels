...
def check_restricted_access(auth):
    """Check if auth have access to restricted endpoints"""
    test_url = 'https://api.pff.com/v1/player/receiving/depth?league=nfl&season=2024&week=1&player_id=78092'
    
    try:
        response = pff_get(test_url, auth, timeout=10)
...
def scrape_all_zero_target_players(auth, players):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        df = scrape_all_zero_target_players(auth, players)
