# Audit 2017 - find suspicious files that might be partial scrapes
# Compares each parquet's row count to what Athena says SHOULD be in that week/half

$ErrorActionPreference = 'Stop'

Write-Host "Fetching all 2017 parquet files from S3..." -ForegroundColor Cyan
$files = aws s3 ls s3://nfl-pff-data-lucas/data/receiving_depth_weekly_no_targets_v2_raw/season=2017/ --recursive | ForEach-Object {
...
Write-Host "Found $($files.Count) parquet files for 2017" -ForegroundColor Cyan
Write-Host ""
