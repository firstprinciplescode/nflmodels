# Audit 2022 - find suspicious files that might be partial scrapes
# Compares each parquet's row count to what Athena says SHOULD be in that week/half

$ErrorActionPreference = 'Stop'

Write-Host "Fetching all 2022 parquet files from S3..." -ForegroundColor Cyan
$files = aws s3 ls s3://nfl-pff-data-lucas/data/receiving_depth_weekly_no_targets_v2_raw/season=2022/ --recursive | ForEach-Object {
...
Write-Host "Found $($files.Count) parquet files for 2022" -ForegroundColor Cyan
Write-Host ""
