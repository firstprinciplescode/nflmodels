...
df_blitz_agg_reg  <- df_blitz_agg_reg %>% arrange(def_ssn, team_name, Season, Week)

start_time <- Sys.time()

xtd_blitz_df <- data.frame()
for(i in 1:nrow(df_blitz_agg_reg)){
  def_ssn2 = df_blitz_agg_reg[i,]$def_ssn
  df_press = df_blitz_agg_reg[-i,]
  df_good <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, no_blitz_snaps, blitz_snaps, DefPerformance, mixed_xtd, qbgrp_ssn, OffPerformance)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T)) 
  df_bad <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, no_blitz_snaps, blitz_snaps, DefPerformance, mixed_xtd, qbgrp_ssn, OffPerformance)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T)) 
  ...
  df_good2 <- cbind(rel_ssn, df_good2)
  df_bad2 <- cbind(rel_ssn, df_bad2)
  df_diff2 <- cbind(rel_ssn, df_diff2)
  colnames(df_good2) <- paste0(colnames(df_good2), "_Good")
  colnames(df_bad2) <- paste0(colnames(df_bad2), "_Bad")
  xtd_blitz_df <- rbind(xtd_blitz_df, cbind(df_good2 %>% filter(def_ssn_Good == def_ssn2), df_bad2 %>% filter(def_ssn_Bad == def_ssn2), df_diff2 %>% filter(def_ssn == def_ssn2)) %>% select(-c(def_ssn_Bad, def_ssn)))
  print(paste(i, def_ssn2))
}
...
xtd_blitz_df2 <- xtd_blitz_df %>% select(-c(def_ssn_Good)) %>% scale() %>% as.data.frame()
...
gamma_press_off = 13 # 13
max_depth_press_off = 5 # 5
min_child_weight_press_off = 3 # 3
alpha_press_off = .1 # .1
lambda_press_off = .65 # .65
colsample_bynode_press_off = .7 # .7
colsample_bylevel_press_off = .7 # .7
colsample_bytree_press_off = .3 # .3
...
write.csv(importance_matrix_press_off, 'imp_blitz_def.csv')
...
df_blitz_good <- df_blitz_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, no_blitz_snaps, blitz_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% summarize(across(everything(), mean, na.rm = T)) 
df_blitz_bad <- df_blitz_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, no_blitz_snaps, blitz_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% summarize(across(everything(), mean, na.rm = T)) 
...
df_blitz_def_scaled_z <- cbind(df_blitz_good$def_ssn, df_blitz_good2, df_blitz_bad2, df_blitz_diff2)
colnames(df_blitz_def_scaled_z)[1] <- "def_ssn"


# Function to comblitzre defenses across seasons and teams
...
dependencies_blitz_def <- list(
  importance_matrix_blitz_def <- importance_matrix_press_off,
  df_blitz_def_scaled_z <- df_blitz_def_scaled_z
)

save(dependencies_blitz_def, file = 'dependencies_blitz_def.RData')

# Example usage
comparison_blitz_def_func("PHI2024", .905)

saveRDS(comparison_blitz_def_func, file = "comparison_blitz_def_func.rds")
