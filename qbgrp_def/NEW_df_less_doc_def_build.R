...
df_less_agg_reg  <- df_less_agg_reg %>% arrange(def_ssn, team_name, Season, Week)

# xtd_less_df <- data.frame()
for(i in 1:nrow(df_less_agg_reg)){
  rel_ssn = df_less_agg_reg[i,]$def_ssn
  df_press = df_less_agg_reg[-i,]
  df_good <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, more_snaps, less_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  df_bad <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, more_snaps, less_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  ...
  xtd_less_df <- rbind(xtd_less_df, cbind(df_good2 %>% filter(def_ssn_Good == rel_ssn), df_bad2 %>% filter(def_ssn_Bad == rel_ssn), df_diff2 %>% filter(def_ssn == rel_ssn)) %>% select(-c(def_ssn_Bad, def_ssn)))
...
xtd_less_df2 <- xtd_less_df %>% select(-c(def_ssn_Good))
xtd_less_df2 <- xtd_less_df2 %>% scale() %>% as.data.frame()
...
df_less_good <- df_less_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, more_snaps, less_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
df_less_bad <- df_less_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, more_snaps, less_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
df_less_diff <- df_less_good[,-1] - df_less_bad[,-1]
colnames(df_less_diff) <- paste0(colnames(df_less_diff), "_diff")
df_less_good2 <- df_less_good[,-1] %>% as.data.frame() %>% scale() %>% as.data.frame()
df_less_bad2 <- df_less_bad[,-1] %>% as.data.frame() %>% scale() %>% as.data.frame()
df_less_diff2 <- df_less_diff %>% as.data.frame() %>% scale() %>% as.data.frame()
colnames(df_less_good2) <- paste0(colnames(df_less_good2), "_Good")
colnames(df_less_bad2) <- paste0(colnames(df_less_bad2), "_Bad")

df_less_def_scaled_z <- cbind(df_less_good$def_ssn, df_less_good2, df_less_bad2, df_less_diff2)
colnames(df_less_def_scaled_z)[1] <- "def_ssn"
...
list_dependencies_less_def <- list(
  ...
  importance_matrix_less_def <- importance_matrix_press_off
)

save(list_dependencies_less_def, file = 'dependencies_less_def.RData')
