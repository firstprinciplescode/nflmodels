...
for(i in 1:nrow(df_depth_agg_reg)){
  ...
  df_good <- df_press %>% as.data.frame() %>% filter(OffPerformance == "Good") %>% select(-c(team_name, Week, Season, short_snaps, medium_snaps, deep_snaps, behind_los_snaps, OffPerformance, DefPerformance, mixed_xtd, def_ssn)) %>% ungroup() %>% group_by(qbgrp_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  df_bad <- df_press %>% as.data.frame() %>% filter(OffPerformance == "Bad") %>% select(-c(team_name, Week, Season, short_snaps, medium_snaps, deep_snaps, behind_los_snaps, OffPerformance, DefPerformance, mixed_xtd, def_ssn)) %>% ungroup() %>% group_by(qbgrp_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  ...
  xtd_depth_df <- rbind(xtd_depth_df, cbind(df_good2 %>% filter(qbgrp_ssn_Good == rel_ssn), df_bad2 %>% filter(qbgrp_ssn_Bad == rel_ssn), df_diff2 %>% filter(qbgrp_ssn == rel_ssn)) %>% select(-c(qbgrp_ssn_Bad, qbgrp_ssn)))
  print(paste(i, rel_ssn))
}

#xtd_depth_df  <- xtd_behind_los_df[1:4145,]
...
xtd_depth_df2 <- xtd_depth_df %>% select(-c(qbgrp_ssn_Good, behind_los_btt_rate_diff, behind_los_btt_rate_Bad, behind_los_btt_rate_Good))
...
min_child_weight_press_off = 25 # 25
alpha_press_off = 1 # 1
lambda_press_off = .2 # .2
colsample_bynode_press_off = 1 # 1
colsample_bylevel_press_off = .25 # .25
colsample_bytree_press_off = .95 # .95
...
df_depth_good <- df_depth_agg_reg %>% as.data.frame() %>% filter(OffPerformance == "Good") %>% select(-c(team_name, Week, Season, behind_los_snaps, short_snaps, medium_snaps, deep_snaps, OffPerformance, DefPerformance, mixed_xtd, def_ssn)) %>% ungroup() %>% group_by(qbgrp_ssn) %>% summarize(across(everything(), mean, na.rm = T))
df_depth_bad <- df_depth_agg_reg %>% as.data.frame() %>% filter(OffPerformance == "Bad") %>% select(-c(team_name, Week, Season, behind_los_snaps, short_snaps, medium_snaps, deep_snaps, OffPerformance, DefPerformance, mixed_xtd, def_ssn)) %>% ungroup() %>% group_by(qbgrp_ssn) %>% summarize(across(everything(), mean, na.rm = T))
...
df_depth_scaled_z <- cbind(df_depth_good$qbgrp_ssn, df_depth_good2, df_depth_bad2, df_depth_diff2)
colnames(df_depth_scaled_z)[1] <- "qbgrp_ssn"
...
list_dependencies_depth <- list(
  ...
  importance_matrix_depth_off <- importance_matrix_press_off
)
