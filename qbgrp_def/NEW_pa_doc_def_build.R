...
df_pa_agg_reg  <- df_pa_agg_reg %>% arrange(def_ssn, team_name, Season, Week)

start_time <- Sys.time()

# xtd_pa_df <- data.frame()
for(i in 1:nrow(df_pa_agg_reg)){
  rel_ssn = df_pa_agg_reg[i,]$def_ssn
  df_press = df_pa_agg_reg[-i,]
  df_good <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, npa_snaps, pa_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  df_bad <- df_press %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, npa_snaps, pa_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
  ...
  xtd_pa_df <- rbind(xtd_pa_df, cbind(df_good2 %>% filter(def_ssn_Good == rel_ssn), df_bad2 %>% filter(def_ssn_Bad == rel_ssn), df_diff2 %>% filter(def_ssn == rel_ssn)) %>% select(-c(def_ssn_Bad, def_ssn)))
...
xtd_pa_df2 <- xtd_pa_df %>% select(-c(def_ssn_Good)) %>% scale() %>% as.data.frame()
...
gamma_press_off = 8.25 # 8.25
max_depth_press_off = 6 # 6
min_child_weight_press_off = 1 # 1
alpha_press_off = .1 # .1
lambda_press_off = 0 # 0
colsample_bynode_press_off = .2 # .2
colsample_bylevel_press_off = .35 # .35
colsample_bytree_press_off = .25 # .35
...
df_pa_good <- df_pa_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Good") %>% select(-c(team_name, Week, Season, npa_snaps, pa_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
df_pa_bad <- df_pa_agg_reg %>% as.data.frame() %>% filter(DefPerformance == "Bad") %>% select(-c(team_name, Week, Season, npa_snaps, pa_snaps, OffPerformance, DefPerformance, mixed_xtd, qbgrp_ssn)) %>% ungroup() %>% group_by(def_ssn) %>% dplyr::summarize(across(everything(), mean, na.rm = T))
df_pa_diff <- df_pa_good[,-1] - df_pa_bad[,-1]
colnames(df_pa_diff) <- paste0(colnames(df_pa_diff), "_diff")
df_pa_good2 <- df_pa_good[,-1] %>% as.data.frame() %>% scale() %>% as.data.frame()
df_pa_bad2 <- df_pa_bad[,-1] %>% as.data.frame() %>% scale() %>% as.data.frame()
df_pa_diff2 <- df_pa_diff %>% as.data.frame() %>% scale() %>% as.data.frame()
colnames(df_pa_good2) <- paste0(colnames(df_pa_good2), "_Good")
colnames(df_pa_bad2) <- paste0(colnames(df_pa_bad2), "_Bad")

df_pa_def_scaled_z <- cbind(df_pa_good$def_ssn, df_pa_good2, df_pa_bad2, df_pa_diff2)
colnames(df_pa_def_scaled_z)[1] <- "def_ssn"
...
list_dependencies_pa_def <- list(
  df_pa_def_scaled_z <- df_pa_def_scaled_z,
  importance_matrix_pa_def <- importance_matrix_press_off
)
