...
# Apply the prediction function
prediction_results_time_to_throw <- make_predictions_time_to_throw(part_nfl)

# Merge predictions back to the original part_nfl dataframe
part_nfl <- part_nfl %>%
  left_join(prediction_results_time_to_throw %>% select(nflverse_game_id, play_id, predicted_time_to_throw), 
            by = c("nflverse_game_id", "play_id")) %>%
  mutate(
    time_to_throw = ifelse(
      is.na(time_to_throw) & (!is.na(air_yards) | !is.na(ngs_air_yards)) & sack == 0 & fumble == 0,
      predicted_time_to_throw,
      time_to_throw
    )
  )


part_nfl$yards_gained <- ifelse(is.na(part_nfl$yards_gained), part_nfl$sack, part_nfl$yards_gained)
...
# FOR YPA ... I'M RIGHT NOW SET ON DOING TWO YPAS - ONE AT SNAP, WITH PASS RUSHER AND BACK - OTHER WAY EVERYTHING ELSE. FOR EXAMPLE, TIME TO THROW AND THE ROUTE SHIT IS GOING TO MASSIVELY BE MASSIVELY INFLUENTIAL. FUN. 


xgboost_part_nfl_snap <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(yards_gained) & pass_attempt == 1 & rush_attempt == 0) %>% select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind)
# QB_SCRAMBLE MAY THROW THINGS OFF - JK, yards_gained IS NULL
# DON'T HAVE TIME_TO_THROW OR THE ROUTE STUFF - TOO MUCH TIME_TO_THROW INFORMING PRESSURE, VICE VERSA. NO SHIT IF YOU HOLD ONTO THE BALL YOU'RE GOING TO BE UNDER PRESSURE. LET'S HAVE TWO MODELS ACTUALLY

xgboost_part_nfl_throw <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(yards_gained) & pass_attempt == 1 & rush_attempt == 0) %>% select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind, ngs_air_yards, pressure_ind)


# WEATHER JUST IN CASE

xgboost_part_nfl_snap_weather <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp) & !is.na(yards_gained) & pass_attempt == 1 & rush_attempt == 0) %>% select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, temp, wind, pressure_ind)

xgboost_part_nfl_throw_weather <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp) & !is.na(yards_gained) & pass_attempt == 1 & rush_attempt == 0) %>% select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, temp, wind, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind, ngs_air_yards, pressure_ind)


##### XGBOOST ONE


sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_snap$yards_gained, SplitRatio = 0.8)
train_set_pass_all_dfs <- subset(x = xgboost_part_nfl_snap, sample_split_all_dfs_pass == TRUE)
test_set_pass_all_dfs <- subset(x = xgboost_part_nfl_snap, sample_split_all_dfs_pass == FALSE)
...
d_xpass_all = xgboost_part_nfl_snap %>% select(-yards_gained)
d_ypass_all = xgboost_part_nfl_snap$yards_gained
...
eta_part_pass = .015 # .015
gamma_part_pass = 4 # 4
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 6 # 6
alpha_part_pass = .3 # .3
lambda_part_pass = 0 # 0
colsample_bynode_part_pass = .3 # .3
colsample_bylevel_part_pass = .8 # .8
colsample_bytree_part_pass = .8 # .8
...
xgb.save(xgb_part_pass, 'xgb_part_ypa_snap_no_weather.model')
...
gamma_part_pass = 0 # 0
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 4 # 4
alpha_part_pass = 0 # 0
lambda_part_pass = .15 # .15
colsample_bynode_part_pass = .2 # .2
...
xgb.save(xgb_part_pass, 'xgb_part_ypa_throw_no_weather.model')
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_snap_weather$yards_gained, SplitRatio = 0.8)
train_set_pass_all_dfs <- subset(x = xgboost_part_nfl_snap_weather, sample_split_all_dfs_pass == TRUE)
test_set_pass_all_dfs <- subset(x = xgboost_part_nfl_snap_weather, sample_split_all_dfs_pass == FALSE)
...
d_xpass_all = xgboost_part_nfl_snap_weather %>% select(-yards_gained)
d_ypass_all = xgboost_part_nfl_snap_weather$yards_gained
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)

watchlist_pass_all_dfs = list(train=dtrain_pass_all_dfs, test=dtest_pass_all_dfs)


eta_part_pass = .023 # .023
gamma_part_pass = 0 # 0
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 9 # 9
alpha_part_pass = 0 # 0
lambda_part_pass = .3 # .3
colsample_bynode_part_pass = .3 # .3
colsample_bylevel_part_pass = 1 # 1
colsample_bytree_part_pass = 1 # 1
...
xgb.save(xgb_part_pass, 'xgb_part_ypa_snap_weather.model')


#####
##### # WEATHER IS PRETTY DECENT ADD FOR ABOVE
#####

### NOT NECESSARY BELOW


sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_throw_weather$yards_gained, SplitRatio = 0.8)
train_set_pass_all_dfs <- subset(x = xgboost_part_nfl_throw_weather, sample_split_all_dfs_pass == TRUE)
test_set_pass_all_dfs <- subset(x = xgboost_part_nfl_throw_weather, sample_split_all_dfs_pass == FALSE)
...
d_xpass_all = xgboost_part_nfl_throw_weather %>% select(-yards_gained)
d_ypass_all = xgboost_part_nfl_throw_weather$yards_gained
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)

watchlist_pass_all_dfs = list(train=dtrain_pass_all_dfs, test=dtest_pass_all_dfs)


eta_part_pass = .035 # .035
gamma_part_pass = 1.5 # 1.5
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 4 # 4
alpha_part_pass = .4 # .2
lambda_part_pass = .15 # .15
...
xgb.save(xgb_part_pass, 'xgb_part_ypa_throw_weather.model')
