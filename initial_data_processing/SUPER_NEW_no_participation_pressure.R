...
xgboost_pbp_snap$pressure_ind <- as.factor(xgboost_pbp_snap$pressure_ind)
xgboost_pbp_snap_weather$pressure_ind <- as.factor(xgboost_pbp_snap_weather$pressure_ind)
...
# Define task
task_pbp_pressure <- TaskClassif$new(
  id = "pbp_run_no_temp_task",
  backend = xgboost_pbp_snap,
  target = "pressure_ind"
)

# Define binary classification learner
learner_pbp_pressure <- lrn("classif.xgboost", predict_type = "prob")
...
instance <- TuningInstanceBatchSingleCrit$new(
  task = task_pbp_pressure,  # should be TaskClassif
  learner = learner_pbp_pressure,
...
xgboost_pbp_snap$pressure_ind <- as.integer(as.factor(xgboost_pbp_snap$pressure_ind)) - 1


sample_split_pbp <- sample.split(Y = xgboost_pbp_snap$pressure_ind, SplitRatio = 0.8)
train_set_pbp <- subset(x = xgboost_pbp_snap, sample_split_pbp == TRUE)
test_set_pbp <- subset(x = xgboost_pbp_snap, sample_split_pbp == FALSE)

train_set_pbp <- train_set_pbp %>% filter(!is.na(pressure_ind))
test_set_pbp <- test_set_pbp %>% filter(!is.na(pressure_ind))

X_train_pbp <- train_set_pbp %>% select(-pressure_ind) %>% as.data.frame()
X_test_pbp <- test_set_pbp %>% select(-pressure_ind) %>% as.data.frame()
y_train_pbp <- train_set_pbp$pressure_ind
y_test_pbp <- test_set_pbp$pressure_ind
...
d_xpbp_all = xgboost_pbp_snap %>% select(-pressure_ind)
d_ypbp_all = xgboost_pbp_snap$pressure_ind
...
eta_pbp_pbp = .005 # .094
gamma_pbp_pbp = 6.656 # .125
max_depth_pbp_pbp = 7 # 5
min_child_weight_pbp_pbp = 4.208 # 0
alpha_pbp_pbp = .812 # 0
lambda_pbp_pbp = .454 # 0
colsample_bynode_pbp_pbp = .531 # .85
colsample_bylevel_pbp_pbp = .849 # 1
colsample_bytree_pbp_pbp = .695 # 1
...
xgb.save(xgb_pbp_pbp, 'xgb_pbp_pressure_no_weather.model')
...
# Define task
task_pbp_pressure_weather <- TaskClassif$new(
  id = "pbp_run_no_temp_task",
  backend = xgboost_pbp_snap_weather,
  target = "pressure_ind"
)
...
param_set <- ps(
  ...
  min_child_weight = p_dbl(lower = 0, upper = 18),
  alpha = p_dbl(lower = 0, upper = 1),
  lambda = p_dbl(lower = 0, upper = 1),
  colsample_bynode = p_dbl(lower = 0, upper = 1),
  colsample_bylevel = p_dbl(lower = 0, upper = 1),
  colsample_bytree = p_dbl(lower = 0, upper = 1),
  nrounds = p_int(lower = 100, upper = 2700)
)

# Define tuning instance (optional standalone)
instance <- TuningInstanceBatchSingleCrit$new(
  task = task_pbp_pressure_weather,  # should be TaskClassif
  learner = learner_pbp_pressure,
  resampling = rsmp("holdout"),
  measure = msr("classif.auc"),  # or msr("classif.logloss")
  search_space = param_set,
...
# Train the tuning instance
at$train(task_pbp_pressure_weather)
...
# Make sure it's a factor, then use as.integer() - 1
xgboost_pbp_snap_weather$pressure_ind <- as.integer(as.factor(xgboost_pbp_snap_weather$pressure_ind)) - 1


sample_split_pbp_weather <- sample.split(Y = xgboost_pbp_snap_weather$pressure_ind, SplitRatio = 0.8)
train_set_pbp_weather <- subset(x = xgboost_pbp_snap_weather, sample_split_pbp_weather == TRUE)
test_set_pbp_weather <- subset(x = xgboost_pbp_snap_weather, sample_split_pbp_weather == FALSE)

train_set_pbp_weather <- train_set_pbp_weather %>% filter(!is.na(pressure_ind))
test_set_pbp_weather <- test_set_pbp_weather %>% filter(!is.na(pressure_ind))

X_train_pbp_weather <- train_set_pbp_weather %>% select(-pressure_ind) %>% as.data.frame()
X_test_pbp_weather <- test_set_pbp_weather %>% select(-pressure_ind) %>% as.data.frame()
y_train_pbp_weather <- train_set_pbp_weather$pressure_ind
y_test_pbp_weather <- test_set_pbp_weather$pressure_ind
...
d_xpbp_all_weather = xgboost_pbp_snap_weather %>% select(-pressure_ind)
d_ypbp_all_weather = xgboost_pbp_snap_weather$pressure_ind
...
eta_pbp_pbp_weather = .033 # .094
gamma_pbp_pbp_weather = 4.917 # .125
max_depth_pbp_pbp_weather = 3 # 5
min_child_weight_pbp_pbp_weather = 6.717 # 0
alpha_pbp_pbp_weather = .417 # 0
lambda_pbp_pbp_weather = .009 # 0
colsample_bynode_pbp_pbp_weather = .818 # .85
colsample_bylevel_pbp_pbp_weather = .527 # 1
colsample_bytree_pbp_pbp_weather = .884 # 1
...
resultant_df_pbp_pbp_weather$buckets <- cut(resultant_df_pbp_pbp_weather$Preds, breaks, labels = labels, include.lowest = TRUE, right = FALSE)
...
predictions_weather <- predict(xgb_pbp_pbp_weather, newdata = dtest_pbp_weather, type = "response")
...
xgboost_pbp_snap_comp <- pbp_nfl_pressure %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(pressure_ind) & !is.na(temp)) %>% select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind)


# Make sure it's a factor, then use as.integer() - 1
xgboost_pbp_snap_comp$pressure_ind <- as.integer(as.factor(xgboost_pbp_snap_comp$pressure_ind)) - 1

sample_split_pbp_comp <- sample.split(Y = xgboost_pbp_snap_comp$pressure_ind, SplitRatio = 0.8)
train_set_pbp_comp <- subset(x = xgboost_pbp_snap_comp, sample_split_pbp_comp == TRUE)
test_set_pbp_comp <- subset(x = xgboost_pbp_snap_comp, sample_split_pbp_comp == FALSE)

train_set_pbp_comp <- train_set_pbp_comp %>% filter(!is.na(pressure_ind))
test_set_pbp_comp <- test_set_pbp_comp %>% filter(!is.na(pressure_ind))

X_train_pbp_comp <- train_set_pbp_comp %>% select(-pressure_ind) %>% as.data.frame()
X_test_pbp_comp <- test_set_pbp_comp %>% select(-pressure_ind) %>% as.data.frame()
y_train_pbp_comp <- train_set_pbp_comp$pressure_ind
y_test_pbp_comp <- test_set_pbp_comp$pressure_ind
...
# Log loss
logloss_no_weather <- logLoss(y_test_pbp_comp, predict(xgb_pbp_pbp, newdata = dtest_pbp_comp))
