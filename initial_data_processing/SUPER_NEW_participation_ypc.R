...
xgboost_part_nfl <- part_nfl %>% filter(play_type == "run" & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & pass_attempt == 0 & rush_attempt == 1) %>% select(yards_gained, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, end_ind, guard_ind, tackle_ind)
...
task_pbp_run_yac <- TaskRegr$new(
  id = "pbp_run_yac_task",
  backend = xgboost_part_nfl,  # same data, but make sure `yards_after_catch` is present and numeric
  target = "yards_gained"
)
...
eta_part_pass = .054 # .03
gamma_part_pass = 3.668 # .25
max_depth_part_pass = 2 # 4
min_child_weight_part_pass = 7.166 # 3
alpha_part_pass = .466 # 1
lambda_part_pass = .338 # 1
colsample_bynode_part_pass = .168 # .35
colsample_bylevel_part_pass = .289 # .95
colsample_bytree_part_pass = .393 # .95
...
xgb.save(xgb_part_pass, 'xgb_pbp_ypc.model')
