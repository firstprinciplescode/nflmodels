...
#defenders_in_box_model_beg <- lm(defenders_in_box ~ season_type + yardline_100 + half_seconds_remaining + down + down_one_ind + down_two_ind + mod_ydstogo + shotgun + no_huddle + last_timeout_ind + score_differentialeam + roof + surface + posteam_ind + n_ol + n_te + n_wr + n_rb + n_dl + n_lb + n_db + 0, data = part_nfl %>% filter(!is.na(defenders_in_box)))
#ols_def_in_box <- ols_step_forward_p(defenders_in_box_model_beg, details = TRUE, prem = .1)

defenders_in_box_model_beg <- lm(defenders_in_box ~ n_db + shotgun + mod_ydstogo + n_wr + down_one_ind + score_differential + yardline_100 + n_ol + n_dl + half_seconds_remaining + no_huddle + down_two_ind + last_timeout_ind + n_lb + down + posteam_ind + 0, data = part_nfl %>% filter(!is.na(defenders_in_box)))
summary(defenders_in_box_model_beg)


part_nfl <- part_nfl %>% mutate(
  defenders_in_box = ifelse(is.na(defenders_in_box), round(.257 * n_db - .459 * shotgun - .045 * mod_ydstogo - .265 * n_wr + .336 * down_one_ind + .006 * score_differential - .0026 * yardline_100 + .269 * n_ol + .822 * n_dl + .0006 * half_seconds_remaining - .119 * no_huddle + .144 * down_two_ind - .107 * last_timeout_ind + .776 * n_lb + .066 * down - .009 * posteam_ind, 0), defenders_in_box))
...
part_nfl_ml_weather <- part_nfl %>% filter(two_point_attempt == 0 & fumble == 0 & qb_kneel == 0 & !is.na(defenders_in_box) & !is.na(wind)) %>% select(td_side, yardline_100, defenders_in_box, half_seconds_remaining, shotgun, no_huddle, qb_dropback, posteam_timeouts_remaining, defteam_timeouts_remaining, score_differential, mod_ydstogo, n_ol:n_st_def, wind, temp)
