...
# FOR AT SNAP, LET'S USE THE MODEL FOR TIME TO THROW WITH PASS RUSHERS.
# FOR TIME TO THROW, USE THE SHIT INCLUDING ROUTE

### UPDATE 8/23 - IT CAN ONLY BE AT THE SNAP. IF WE USE TIME_TO_THROW THEN THERE'S A BUNCH OF SACKS THAT GET REMOVED. WELP. 


xgboost_part_nfl_snap <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0) %>% select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db)
# QB_SCRAMBLE MAY THROW THINGS OFF - JK, PRESSURE_IND IS NULL
...
xgboost_part_nfl_time <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0) %>% select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind)


# WEATHER JUST IN CASE

xgboost_part_nfl_snap_weather <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp)) %>% select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, temp, wind)

xgboost_part_nfl_time_weather <- part_nfl %>% filter((play_type == "pass") & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp)) %>% select(pressure_ind, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, temp, wind, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind)


##### XGBOOST ONE


sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_snap$pressure_ind, SplitRatio = 0.8)
...
d_xpass_all = xgboost_part_nfl_snap %>% select(-pressure_ind)
d_ypass_all = xgboost_part_nfl_snap$pressure_ind
...
min_child_weight_part_pass = 2 # 2
alpha_part_pass = 0 # 0
lambda_part_pass = .55 # .55
colsample_bynode_part_pass = .175 # .175
...
xgb.save(xgb_part_pass, 'xgb_part_pressure_snap_no_weather.model')
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_time$pressure_ind, SplitRatio = 0.8)
train_set_pass_all_dfs <- subset(x = xgboost_part_nfl_time, sample_split_all_dfs_pass == TRUE)
test_set_pass_all_dfs <- subset(x = xgboost_part_nfl_time, sample_split_all_dfs_pass == FALSE)
...
d_xpass_all = xgboost_part_nfl_time %>% select(-pressure_ind)
d_ypass_all = xgboost_part_nfl_time$pressure_ind
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)
...
eta_part_pass = .11 # .11
gamma_part_pass = .575 # .575
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 0 # 0
alpha_part_pass = 0 # 0
lambda_part_pass = .9 # .9
colsample_bynode_part_pass = .2 # .2
colsample_bylevel_part_pass = .99 # .99
colsample_bytree_part_pass = .99 # .99
...
xgb.save(xgb_part_pass, 'xgb_part_pressure_throw_no_weather.model')
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_snap_weather$pressure_ind, SplitRatio = 0.8)
...
d_xpass_all = xgboost_part_nfl_snap_weather %>% select(-pressure_ind)
d_ypass_all = xgboost_part_nfl_snap_weather$pressure_ind
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)

watchlist_pass_all_dfs = list(train=dtrain_pass_all_dfs, test=dtest_pass_all_dfs)


eta_part_pass = .075 # .075
gamma_part_pass = 1.5 # 1.5
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 0 # 0
alpha_part_pass = .25 # .25
lambda_part_pass = .825 # .825
colsample_bynode_part_pass = 1 # 1
colsample_bylevel_part_pass = 1 # 1
colsample_bytree_part_pass = 1 # 1
...
xgb.save(xgb_part_pass, 'xgb_part_pressure_snap_weather.model')


#####
#####
#####
# WEATHER SOMETIMES MATTERS. TIME_TO_THROW IS EVERYTHING


sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_time_weather$pressure_ind, SplitRatio = 0.8)
train_set_pass_all_dfs <- subset(x = xgboost_part_nfl_time_weather, sample_split_all_dfs_pass == TRUE)
test_set_pass_all_dfs <- subset(x = xgboost_part_nfl_time_weather, sample_split_all_dfs_pass == FALSE)
...
d_xpass_all = xgboost_part_nfl_time_weather %>% select(-pressure_ind)
d_ypass_all = xgboost_part_nfl_time_weather$pressure_ind
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)
