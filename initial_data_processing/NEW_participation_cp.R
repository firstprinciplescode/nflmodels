# THIS WILL BE FOR PARTICIPATION CP
...
xgboost_part_nfl <- part_nfl %>% filter(play_type == "pass" & pass_attempt == 1 & sack == 0 & two_point_attempt == 0) %>% select(complete_pass, qb_hit, time_to_throw, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind, flat_route_ind,  screen_route_ind, hitch_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, no_route_ind, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, ngs_air_yards)

xgboost_part_nfl_weather <- part_nfl %>% filter(play_type == "pass" & pass_attempt == 1 & sack == 0 & two_point_attempt == 0 & !is.na(wind)) %>% select(complete_pass, qb_hit, time_to_throw, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind, flat_route_ind,  screen_route_ind, hitch_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, no_route_ind, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, ngs_air_yards, wind, temp)
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl$complete_pass, SplitRatio = 0.8)
...
train_set_pass_all_dfs <- train_set_pass_all_dfs %>% filter(!is.na(complete_pass)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind) & !is.na(ngs_air_yards) & !is.na(pressure_ind))
test_set_pass_all_dfs <- test_set_pass_all_dfs %>% filter(!is.na(complete_pass)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind) & !is.na(ngs_air_yards) & !is.na(pressure_ind))
...
d_xpass_all = xgboost_part_nfl %>% select(-complete_pass)
d_ypass_all = xgboost_part_nfl$complete_pass
...
colsample_bynode_part_pass = 1 # .275
colsample_bylevel_part_pass = 1 # .95
colsample_bytree_part_pass = 1 # .95
...
xgb.save(xgb_part_pass, 'xgb_part_cp_no_weather.model')
