...
part_nfl <- part_nfl %>% filter(play_type %in% c("pass") & two_point_attempt == 0)


# Load pre-trained XGBoost models
xgb_defenders_box_no_weather <- xgb.load("xgb_defenders_box_no_weather.model")
xgb_defenders_box_weather <- xgb.load("xgb_defenders_box_weather.model")
...
make_predictions <- function(part_data) {
  ...
  predictions_with_weather <- predict(xgb_defenders_box_weather, newdata = dtest_with_weather)
  
  # Data without weather info
  dtest_without_weather <- create_dmatrix(part_data)
  predictions_without_weather <- predict(xgb_defenders_box_no_weather, newdata = dtest_without_weather)
  
  # Initialize a new column with NA values
  part_data$predicted_defenders_in_box <- NA
  
  # Assign predictions to the new column based on the availability of weather data
  part_data$predicted_defenders_in_box[!is.na(part_data$temp) & !is.na(part_data$wind)] <- predictions_with_weather
  part_data$predicted_defenders_in_box[is.na(part_data$temp) | is.na(part_data$wind)] <- predictions_without_weather
...
part_nfl <- part_nfl %>%
  left_join(prediction_results_pass_rushers %>% select(nflverse_game_id, play_id, predicted_pass_rushers), 
            by = c("nflverse_game_id", "play_id")) %>%
  mutate(
    number_of_pass_rushers = ifelse(is.na(number_of_pass_rushers), predicted_pass_rushers, number_of_pass_rushers)
  )
...
# Load the models for each scenario
xgb_time_to_throw_before_no_weather <- xgb.load("xgb_time_to_throw_before_no_weather.model")
xgb_time_to_throw_before_weather <- xgb.load("xgb_time_to_throw_before_weather.model")

xgb_time_to_throw_at_snap_no_weather <- xgb.load("xgb_time_to_throw_at_snap_no_weather.model")
xgb_time_to_throw_at_snap_weather <- xgb.load("xgb_time_to_throw_at_snap_weather.model")

xgb_time_to_throw_at_throw_no_weather <- xgb.load("xgb_time_to_throw_route_no_weather.model")
xgb_time_to_throw_at_throw_weather <- xgb.load("xgb_time_to_throw_route_weather.model")
...
create_dmatrix_before_snap <- function(data, weather_cols = NULL) {
  if (!is.null(weather_cols)) {
    data <- data %>% filter(!is.na(!!sym(weather_cols[1])) & !is.na(!!sym(weather_cols[2])))
  } else {
    data <- data %>% filter(is.na(temp) | is.na(wind))
  ...
  X <- data %>% select(
    ...
    mod_ydstogo, shotgun, no_huddle, last_timeout_ind, score_differential, posteam_timeouts_remaining,
    defteam_timeouts_remaining, roof, surface, posteam_ind, n_ol, n_te, n_wr, n_rb, n_dl, n_lb, n_db,
    defenders_in_box, temp, wind
  ) %>% select_if(~ !all(is.na(.))) %>% as.data.frame()
  
  return(xgb.DMatrix(data = as.matrix(X)))
}

create_dmatrix_at_snap <- function(data, weather_cols = NULL) {
  if (!is.null(weather_cols)) {
    data <- data %>% filter(!is.na(!!sym(weather_cols[1])) & !is.na(!!sym(weather_cols[2])))
  } else {
    data <- data %>% filter(is.na(temp) | is.na(wind))
  ...
  X <- data %>% select(
    ...
    mod_ydstogo, shotgun, no_huddle, last_timeout_ind, score_differential, posteam_timeouts_remaining,
    defteam_timeouts_remaining, roof, surface, posteam_ind, n_ol, n_te, n_wr, n_rb, n_dl, n_lb, n_db,
    defenders_in_box, number_of_pass_rushers, temp, wind
  ) %>% select_if(~ !all(is.na(.))) %>% as.data.frame()
  
  return(xgb.DMatrix(data = as.matrix(X)))
}

create_dmatrix_at_throw <- function(data, weather_cols = NULL) {
  if (!is.null(weather_cols)) {
    data <- data %>% filter(!is.na(!!sym(weather_cols[1])) & !is.na(!!sym(weather_cols[2])))
  } else {
    data <- data %>% filter(is.na(temp) | is.na(wind))
  ...
  X <- data %>% select(
    season_type , yardline_100 , half_seconds_remaining , down , down_one_ind , down_two_ind , down_three_ind , mod_ydstogo , shotgun , no_huddle , last_timeout_ind , score_differential , posteam_timeouts_remaining , defteam_timeouts_remaining , roof , surface , posteam_ind , n_ol , n_te , n_wr , n_rb , n_dl , n_lb , n_db, defenders_in_box, temp, wind, number_of_pass_rushers, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind, temp, wind
  ) %>% select_if(~ !all(is.na(.))) %>% as.data.frame()
...
make_predictions_before_snap <- function(part_data) {
  # Data with weather info
  dtest_with_weather <- create_dmatrix_before_snap(part_data, c("temp", "wind"))
  predictions_with_weather <- predict(xgb_time_to_throw_before_weather, newdata = dtest_with_weather)
  
  # Data without weather info
  dtest_without_weather <- create_dmatrix_before_snap(part_data)
  predictions_without_weather <- predict(xgb_time_to_throw_before_no_weather, newdata = dtest_without_weather)
  
  # Initialize a new column with NA values
  part_data$predicted_time_to_throw_before_snap <- NA
  
  # Assign predictions to the new column based on the availability of weather data
  part_data$predicted_time_to_throw_before_snap[!is.na(part_data$temp) & !is.na(part_data$wind)] <- predictions_with_weather
  part_data$predicted_time_to_throw_before_snap[is.na(part_data$temp) | is.na(part_data$wind)] <- predictions_without_weather
  
  return(part_data)
}

make_predictions_at_snap <- function(part_data) {
  # Data with weather info
  dtest_with_weather <- create_dmatrix_at_snap(part_data, c("temp", "wind"))
  predictions_with_weather <- predict(xgb_time_to_throw_at_snap_weather, newdata = dtest_with_weather)
  
  # Data without weather info
  dtest_without_weather <- create_dmatrix_at_snap(part_data)
  predictions_without_weather <- predict(xgb_time_to_throw_at_snap_no_weather, newdata = dtest_without_weather)
  
  # Initialize a new column with NA values
  part_data$predicted_time_to_throw_at_snap <- NA
  
  # Assign predictions to the new column based on the availability of weather data
  part_data$predicted_time_to_throw_at_snap[!is.na(part_data$temp) & !is.na(part_data$wind)] <- predictions_with_weather
  part_data$predicted_time_to_throw_at_snap[is.na(part_data$temp) | is.na(part_data$wind)] <- predictions_without_weather
  
  return(part_data)
}

make_predictions_at_throw <- function(part_data) {
  # Data with weather info
  dtest_with_weather <- create_dmatrix_at_throw(part_data, c("temp", "wind"))
  predictions_with_weather <- predict(xgb_time_to_throw_at_throw_weather, newdata = dtest_with_weather)
  
  # Data without weather info
  dtest_without_weather <- create_dmatrix_at_throw(part_data)
  predictions_without_weather <- predict(xgb_time_to_throw_at_throw_no_weather, newdata = dtest_without_weather)
  
  # Initialize a new column with NA values
  part_data$predicted_time_to_throw_at_throw <- NA
  
  # Assign predictions to the new column based on the availability of weather data
  part_data$predicted_time_to_throw_at_throw[!is.na(part_data$temp) & !is.na(part_data$wind)] <- predictions_with_weather
  part_data$predicted_time_to_throw_at_throw[is.na(part_data$temp) | is.na(part_data$wind)] <- predictions_without_weather
  
  # Coalesce with existing data only if conditions are met
  part_data$time_to_throw <- ifelse(is.na(part_data$time_to_throw) & !(part_data$sack == 1 | part_data$fumble == 1 | is.na(part_data$route) | (is.na(part_data$air_yards) & is.na(part_data$ngs_air_yards))), 
                                    part_data$predicted_time_to_throw_at_throw, 
                                    part_data$time_to_throw)
...
# Apply predictions before snap
prediction_results_before_snap <- make_predictions_before_snap(part_nfl)

# Apply predictions at snap
prediction_results_at_snap <- make_predictions_at_snap(part_nfl)

# Apply predictions at throw
prediction_results_at_throw <- make_predictions_at_throw(part_nfl)


time_to_throw_results_df <- data.frame(cbind(prediction_results_before_snap$nflverse_game_id, prediction_results_before_snap$play_id, prediction_results_before_snap$predicted_time_to_throw_before_snap, prediction_results_at_snap$predicted_time_to_throw_at_snap, prediction_results_at_throw$predicted_time_to_throw_at_throw))


# Merge predictions back to original part_nfl dataframe
part_nfl <- part_nfl %>%
  left_join(prediction_results_before_snap %>% select(nflverse_game_id, play_id, predicted_time_to_throw_before_snap), 
            by = c("nflverse_game_id", "play_id"))
...
eta_part_pass = .025 # .025
gamma_part_pass = 0 # 0
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 0 # 0
alpha_part_pass = 0 # 0
lambda_part_pass = 0 # 0
colsample_bynode_part_pass = .275 # .275
colsample_bylevel_part_pass = 1 # 1
colsample_bytree_part_pass = 1 # 1
...
xgb.save(xgb_part_pass, 'xgb_part_after_pass_xtd.model')


#### THIS IS DONE. GOING WITH IT. 
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_weather$td_side, SplitRatio = 0.8)
