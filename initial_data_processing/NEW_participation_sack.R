...
#### AT THE MOMENT OF SNAP - WHAT IS SET IN STONE? NO ROUTES, FOR EXAMPLE. COVERAGES? LET'S GO YES. 


xgboost_part_nfl <- part_nfl %>% filter(play_type == "pass" & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0) %>% select(sack, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db)

xgboost_part_nfl_weather <- part_nfl %>% filter(play_type == "pass" & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp)) %>% select(sack, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, temp, wind)
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl$sack, SplitRatio = 0.8)
...
train_set_pass_all_dfs <- train_set_pass_all_dfs %>% filter(!is.na(sack)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind))
test_set_pass_all_dfs <- test_set_pass_all_dfs %>% filter(!is.na(sack)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind))

X_train_pass_all_dfs <- train_set_pass_all_dfs %>% select(-sack) %>% as.data.frame()
X_test_pass_all_dfs <- test_set_pass_all_dfs %>% select(-sack) %>% as.data.frame()
y_train_pass_all_dfs <- train_set_pass_all_dfs$sack
y_test_pass_all_dfs <- test_set_pass_all_dfs$sack
...
d_xpass_all = xgboost_part_nfl %>% select(-sack)
d_ypass_all = xgboost_part_nfl$sack
...
eta_part_pass = .07 # .07
gamma_part_pass = 3.75 # 3.75
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 1 # 1
alpha_part_pass = .3 # .3
...
xgb.save(xgb_part_pass, 'xgb_part_sack_no_weather.model')
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_weather$sack, SplitRatio = 0.8)
...
d_xpass_all = xgboost_part_nfl_weather %>% select(-sack)
d_ypass_all = xgboost_part_nfl_weather$sack
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)
...
gamma_part_pass = 1 # 1
max_depth_part_pass = 3 # 3
min_child_weight_part_pass = 0 # 0
alpha_part_pass = 0 # 0
lambda_part_pass = .25 # .25
colsample_bynode_part_pass = .5 # .5
colsample_bylevel_part_pass = .625 # .5
colsample_bytree_part_pass = .625 # .5
...
### WEATHER DOES MATTER. INTERESTING-ISH
...
xgb.save(xgb_part_pass, 'xgb_part_sack_weather.model')
