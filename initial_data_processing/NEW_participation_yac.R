...
part_nfl$yards_after_catch <- ifelse(is.na(part_nfl$yards_after_catch), part_nfl$sack, part_nfl$yards_after_catch)
...
xgboost_part_nfl_throw <- part_nfl %>% filter((play_type == "pass") & complete_pass == 1 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(yards_after_catch) & pass_attempt == 1 & rush_attempt == 0 & !is.na(yards_after_catch)) %>% select(yards_after_catch, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind, ngs_air_yards, pressure_ind)

xgboost_part_nfl_throw_weather <- part_nfl %>% filter((play_type == "pass") & complete_pass == 1 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(temp) & !is.na(yards_after_catch) & pass_attempt == 1 & rush_attempt == 0 & !is.na(yards_after_catch)) %>% select(yards_after_catch, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, cover3_ind, cover2_ind, cover0_ind, cover1_ind, cover4_ind, twoman_ind, cover6_ind, time_to_throw, temp, wind, hitch_route_ind, go_route_ind, out_route_ind, slant_route_ind, cross_route_ind, post_route_ind, corner_route_ind, in_route_ind, wheel_route_ind, ngs_air_yards, pressure_ind)
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_throw$yards_after_catch, SplitRatio = 0.8)
...
train_set_pass_all_dfs <- train_set_pass_all_dfs %>% filter(!is.na(yards_after_catch)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind))
test_set_pass_all_dfs <- test_set_pass_all_dfs %>% filter(!is.na(yards_after_catch)) %>% filter(!is.na(score_differential) & !is.na(down_three_ind))
...
d_xpass_all = xgboost_part_nfl_throw %>% select(-yards_after_catch)
d_ypass_all = xgboost_part_nfl_throw$yards_after_catch
...
eta_part_pass = .041 # .041
gamma_part_pass = 0 # 0
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 10 # 10
alpha_part_pass = .2 # .2
...
xgb.save(xgb_part_pass, 'xgb_part_yac_no_weather.model')
...
sample_split_all_dfs_pass <- sample.split(Y = xgboost_part_nfl_throw_weather$yards_after_catch, SplitRatio = 0.8)
...
d_xpass_all = xgboost_part_nfl_throw_weather %>% select(-yards_after_catch)
d_ypass_all = xgboost_part_nfl_throw_weather$yards_after_catch
d_all_pass_all = xgb.DMatrix(data = as.matrix(d_xpass_all), label = d_ypass_all)

watchlist_pass_all_dfs = list(train=dtrain_pass_all_dfs, test=dtest_pass_all_dfs)


eta_part_pass = .042 # .042
gamma_part_pass = 0 # 0
max_depth_part_pass = 4 # 4
min_child_weight_part_pass = 7 # 7
alpha_part_pass = .2 # .2
lambda_part_pass = 0 # 0
colsample_bynode_part_pass = 1 # 1
colsample_bylevel_part_pass = 1 # 1
...
xgb.save(xgb_part_pass, 'xgb_part_yac_weather.model')
