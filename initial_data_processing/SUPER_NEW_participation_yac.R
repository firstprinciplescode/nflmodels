...
xgboost_part_nfl_throw <- part_nfl %>% filter((play_type == "pass") & complete_pass == 1 & qb_spike == 0 & qb_kneel == 0 & two_point_attempt == 0 & !is.na(yards_after_catch) & pass_attempt == 1 & rush_attempt == 0 & !is.na(yards_after_catch)) %>% select(yards_after_catch, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, ngs_air_yards, pressure_ind)
...
eta_pbp_pass = .095 # .036
gamma_pbp_pass = 2.114 # 0
max_depth_pbp_pass = 7 # 4
min_child_weight_pbp_pass = 3.973 # 2
alpha_pbp_pass = .502 # .2
lambda_pbp_pass = .836 # 0
colsample_bynode_pbp_pass = .927 # .6
colsample_bylevel_pbp_pass = .848 # .9
colsample_bytree_pbp_pass = .604 # .9
