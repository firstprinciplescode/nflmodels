...
sample_split_pbp <- sample.split(Y = part_nfl_run$td_side, SplitRatio = 0.8)
train_set_pbp <- subset(x = part_nfl_run, sample_split_pbp == TRUE)
test_set_pbp <- subset(x = part_nfl_run, sample_split_pbp == FALSE)
...
d_xpbp_all = part_nfl_run %>% select(-td_side)
d_ypbp_all = part_nfl_run$td_side
...
eta_part_pbp = .095 # .095
gamma_part_pbp = 11.25 # 11.25
max_depth_part_pbp = 2 # 2
min_child_weight_part_pbp = 6.5 # 6.5
alpha_part_pbp = .15 # .15
lambda_part_pbp = 1 # 1
colsample_bynode_part_pbp = .525 # .525
colsample_bylevel_part_pbp = .75 # .75
colsample_bytree_part_pbp = .75 # .75
...
xgb.save(xgb_part_pbp, 'xgb_part_after_run_xtd.model')
