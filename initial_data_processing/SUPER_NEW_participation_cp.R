...
xgboost_part_nfl <- part_nfl %>% filter(play_type == "pass" & pass_attempt == 1 & sack == 0 & two_point_attempt == 0) %>% select(complete_pass, qb_hit, time_to_throw, yardline_100, season_type, half_seconds_remaining, down, down_one_ind, down_two_ind, down_three_ind, defenders_in_box, number_of_pass_rushers, mod_ydstogo, shotgun, no_huddle, score_differential, surface, posteam_ind, n_ol, n_te, n_rb, n_wr, n_dl, n_lb, n_db, pressure_ind, ngs_air_yards)

xgboost_part_nfl$complete_pass <- as.factor(xgboost_part_nfl$complete_pass)
...
xgboost_part_nfl$complete_pass <- as.integer(as.factor(xgboost_part_nfl$complete_pass)) - 1
...
eta_part_pass = .084 # .04
gamma_part_pass = 7.274 # 4
max_depth_part_pass = 8 # 4
min_child_weight_part_pass = 3.374 # 3
alpha_part_pass = .201 # 1
lambda_part_pass = .599 # 1
colsample_bynode_part_pass = .763 # .275
colsample_bylevel_part_pass = .627 # .95
colsample_bytree_part_pass = .831 # .95
...
xgb.save(xgb_part_pass, 'xgb_part_cp.model')
