...
xpass_calc_func <- function(qbgrp_one, defgrp_one) {
  ...
  process_category <- function(qbgrp_one, defgrp_one, qb_threshold, def_threshold, qb_func, def_func) {
    ...
    return(rbind(
      qb_stats_df_final %>% 
        ...
        dplyr::summarise(n = n(), across(c(pass_rate, fastr_xpass_rate, pbp_xpass_rate, part_xpass_rate,
                                           pass_rate_rank, pass_rate_rank_def,
                                           fastr_xpass_rate_rank, fastr_xpass_rate_rank_def,
                                           pbp_xpass_rate_rank, pbp_xpass_rate_rank_def,
                                           part_xpass_rate_rank, part_xpass_rate_rank_def), mean, na.rm = TRUE)),
  ...
  # Define sheet name
  sheet_name <- substr(paste0("XPass - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
...
qb_stats_df_final %>% filter(qbgrp_ssn == "CHIWilliams-2025") %>%
  mutate(out_ind = ifelse(week >= 14 & week <= 18, 1, 0)) %>%
  group_by(out_ind) %>%
  dplyr::summarise(mn_pass_rate_rank_def = mean(pass_rate_rank_def, na.rm = T),
                   mn_fastr_xpass_rate_rank_def = mean(fastr_xpass_rate_rank_def, na.rm = T),
                   mn_pbp_xpass_rate_rank_def = mean(pbp_xpass_rate_rank_def, na.rm = T)
                   )

qb_stats_df_final %>% filter(def_ssn == "LA2025") %>%
  mutate(out_ind = ifelse(week >= 17, 1, 0)) %>%
  group_by(out_ind) %>%
  dplyr::summarise(mn_pass_rate_rank = mean(pass_rate_rank_def, na.rm = T),
                   mn_fastr_xpass_rate_rank = mean(fastr_xpass_rate_rank_def, na.rm = T),
                   mn_pbp_xpass_rate_rank = mean(pbp_xpass_rate_rank_def, na.rm = T)
  )
...
qb_stats_df_final %>% filter(def_ssn == "TEN2025") %>%
  select(week, pass_rate, pass_rate_rank, fastr_xpass_rate, fastr_xpass_rate_rank, pbp_xpass_rate, pbp_xpass_rate_rank, part_xpass_rate, part_xpass_rate_rank) %>%
  arrange(week)
...
xpass_calc_func("TBMayfield-2025", "CAR2025")
...
xpass_calc_func("TBMayfield-2024", "CAR2025")
...
# Actual pass rate vs opponent rank
qb_pass_summary <- qb_stats_df_final %>%
  group_by(qbgrp_ssn) %>%
  dplyr::summarise(
    mn_pass_rate = mean(pass_rate, na.rm = T),
    mn_pass_rate_rank_def = mean(pass_rate_rank_def, na.rm = T),
    mn_fastr_xpass_rate = mean(fastr_xpass_rate, na.rm = T),
    mn_fastr_xpass_rate_rank_def = mean(fastr_xpass_rate_rank_def, na.rm = T),
    mn_pbp_xpass_rate = mean(pbp_xpass_rate, na.rm = T),
    mn_pbp_xpass_rate_rank_def = mean(pbp_xpass_rate_rank_def, na.rm = T),
...
# Function to make these plots easily
plot_rate_vs_rank <- function(df, x_var, y_var, x_lab, y_lab, title) {
  fit <- lm(as.formula(paste(y_var, "~", x_var)), data = df)
  coefs <- coef(fit)
  r2 <- summary(fit)$r.squared
  eq_label <- paste0("y = ", round(coefs[1], 3), " + ", round(coefs[2], 3), "x\nR² = ", round(r2, 3))
  ...
  ggplot(df, aes(x = .data[[x_var]], y = .data[[y_var]])) +
    ...
    annotate("text", x = 0.8, y = max(df[[y_var]], na.rm = T) - 0.02, 
             label = eq_label, hjust = 1, size = 4) +
    labs(x = x_lab, y = y_lab, title = title) +
    theme_minimal()
}

# Actual pass rate vs def rank
p1 <- plot_rate_vs_rank(qb_pass_summary, 
                        "mn_pass_rate_rank_def", "mn_pass_rate",
                        "Mean Def Rank Faced (higher = easier vs pass)",
                        "Mean Actual Pass Rate",
                        "Actual Pass Rate vs Percentile Rank")

p1

# fastr xpass rate vs def rank
p2 <- plot_rate_vs_rank(qb_pass_summary,
                        "mn_fastr_xpass_rate_rank_def", "mn_fastr_xpass_rate",
                        "Mean Def Rank Faced (fastr model)",
                        "Mean fastr xPass Rate",
                        "fastr Expected Pass Rate vs Percentile Rank")

p2

# pbp xpass rate vs def rank
p3 <- plot_rate_vs_rank(qb_pass_summary,
                        "mn_pbp_xpass_rate_rank_def", "mn_pbp_xpass_rate",
                        "Mean Def Rank Faced (pbp model)",
                        "Mean pbp xPass Rate",
                        "pbp Expected Pass Rate vs Percentile Rank")
