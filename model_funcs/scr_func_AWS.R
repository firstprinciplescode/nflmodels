  ...
  process_category <- function(qbgrp_one, defgrp_one, qb_threshold, def_threshold, qb_func, def_func) {
    ...
    return(rbind(
      qb_stats_df_final %>% 
        ...
        dplyr::summarise(n = n(), across(c(scr_rate, scr_ypc, pbp_scr_xypc, part_scr_xypc,
...
scr_stats_func("CARYoung-2025", "LA2025")


weather_scr_stats_func <- function(qbgrp_one, defgrp_one) {
  ...
  selected_columns <- c("scr_rate", "scr_ypc", "pbp_scr_xypc", "part_scr_xypc",
  ...
  sheet_name <- substr(paste0("WeatherScr - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
...
weather_scr_stats_func("CARYoung-2025", "LA2025")

precip_scr_stats_func <- function(qbgrp_one, defgrp_one) {
  ...
  sheet_name <- substr(paste0("PrecipScr - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
  put_object(tmp, bucket = bucket, object = paste0("outputs/", sheet_name, ".xlsx"))
...
qb_stats_df_final %>% filter(qbgrp_ssn == "JAXLawrence-2025") %>%
  select(week, scr_rate, scr_rate_rank_def, scr_ypc, scr_ypc_rank_def, pbp_scr_xypc, pbp_scr_xypc_rank_def) %>%
  arrange(week)

qb_stats_df_final %>% filter(def_ssn == "TEN2025") %>%
  select(week, scr_rate, scr_rate_rank, scr_ypc, scr_ypc_rank, pbp_scr_xypc, pbp_scr_xypc_rank) %>%
  arrange(week)
...
scr_stats_func("TENWard-2025", "JAX2025")
...
scr_stats_func("TBMayfield-2025", "CAR2025")
