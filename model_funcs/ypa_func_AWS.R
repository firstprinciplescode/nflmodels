...
ypa_func <- function(qbgrp_one, defgrp_one) {
    ...
    return(rbind(
      qb_stats_df_final %>%
        ...
        dplyr::summarise(n = n(), across(c(
          ypa, pbp_xypa, part_xypa, adot, 
          behind_los_rate, short_rate, medium_rate, deep_rate,
          ypa_rank, ypa_rank_def, pbp_xypa_rank, pbp_xypa_rank_def, 
          part_xypa_rank, part_xypa_rank_def, adot_rank, adot_rank_def,
          behind_los_rate_rank, behind_los_rate_rank_def, 
          short_rate_rank, short_rate_rank_def, 
          medium_rate_rank, medium_rate_rank_def, 
          deep_rate_rank, deep_rate_rank_def), mean, na.rm = TRUE)),
  ...
  sheet_name <- substr(paste0("YPA - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
...
qb_stats_df_final %>% filter(def_ssn == "TB2025") %>%
  select(week, ypa, ypa_rank, pbp_xypa, pbp_xypa_rank) %>%
  arrange(week)
...
ypa_func("TENWard-2025", "JAX2025")


weather_ypa_func <- function(qbgrp_one, defgrp_one) {
  ...
  selected_columns <- c("ypa", "pbp_xypa", "part_xypa", "adot", "behind_los_rate", "short_rate", "medium_rate", "deep_rate",
                        "ypa_rank", "pbp_xypa_rank", "part_xypa_rank", "adot_rank", "behind_los_rate_rank", 
                        "short_rate_rank", "medium_rate_rank", "deep_rate_rank")
  ...
  sheet_name <- substr(paste0("WeatherYPA - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
...
weather_ypa_func("CARYoung-2025", "LA2025")

precip_ypa_func <- function(qbgrp_one, defgrp_one) {
  ...
  sheet_name <- substr(paste0("PrecipYPA - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
  put_object(tmp, bucket = bucket, object = paste0("outputs/", sheet_name, ".xlsx"))
...
precip_ypa_func("CARYoung-2025", "LA2025")
...
ypa_func("TBMayfield-2025", "CAR2025")
