...
keep_objects <- c("qb_stats_df_final", "con", "bucket", "read_athena_query")
...
twp_rate_func <- function(qbgrp_one, defgrp_one) {
  ...
  process_category <- function(qbgrp_one, defgrp_one, qb_threshold, def_threshold, qb_func, def_func) {
    ...
    return(rbind(
      qb_stats_df_final %>% 
        ...
        dplyr::summarise(n = n(), across(c(twp_rate, int_rate, pressure_rate,
                                           twp_rate_rank, twp_rate_rank_def,
                                           int_rate_rank, int_rate_rank_def,
                                           pressure_rate_rank, pressure_rate_rank_def), mean, na.rm = TRUE)),
  ...
  sheet_name <- substr(paste0("TWP - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
...
twp_rate_func("CARYoung-2025", "LA2025")

weather_twp_rate_func <- function(qbgrp_one, defgrp_one) {
  ...
  selected_columns <- c("twp_rate", "int_rate", "pressure_rate",
  ...
  sheet_name <- substr(paste0("WeatherTWP - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
...
weather_twp_rate_func("CARYoung-2025", "LA2025")

precip_twp_rate_func <- function(qbgrp_one, defgrp_one) {
  ...
  sheet_name <- substr(paste0("PrecipTWP - ", qbgrp_one, " vs ", defgrp_one), 1, 31)
  tmp <- tempfile(fileext = ".xlsx")
  saveWorkbook(wb, tmp)
  put_object(tmp, bucket = bucket, object = paste0("outputs/", sheet_name, ".xlsx"))
...
precip_twp_rate_func("CARYoung-2025", "LA2025")
...
qb_stats_df_final %>% filter(def_ssn == "TEN2025") %>%
  select(week, twp_rate, twp_rate_rank, int_rate, int_rate_rank) %>%
  arrange(week)
...
twp_rate_func("TENWard-2025", "JAX2025")
...
twp_rate_func("TBMayfield-2025", "CAR2025")
