...
attach("NEW_pff_rushing_all_clusters_final_workspace.RData")

rushing_stats_final <- rushing_stats_final


# RUN GAP CLUSTERS - NOT ACCURATE
...
# A SITUATION CLUSTERS - NOT ACCURATE

# CLUSTER 1 - LOWER USAGE SECOND / THIRD LONGER YARDAGE
# CLUSTER 2 - MID USAGE EARLIER DOWN LONGER YARDAGE MAYBE
# CLUSTER 3 - LOWER USAGE SECOND / THIRD SHORTER YARDAGE
# CLUSTER 4 - MID USAGE SHORTER YARDAGE
# CLUSTER 5 - HIGH USAGE ALPHA
# CLUSTER 6 - LONG YARDAGE ONLY
# CLUSTER 7 - HIGHER USAGE FIRST DOWN + 2ND PLUS AND SHORT


# B SITUATION CLUSTERS - NOT ACCURATE

# 1 - FIRST DOWN + 2ND PLUS SHORTER YARDAGE
# 2 - LOWER USAGE SHORTER YARDAGE
# 3 - LONGER YARDAGE ONLY
# 4 - LOWER USAGE SHORTER YARDAGE
# 5 - LONGER YARDAGE-ISH, LOWER USAGE
# 6 - FIRST / SECOND DOWN


# C SITUATION CLUSTERS - NOT ACCURATE

# 1 - MID USAGE SHORTER YARDAGE
# 2 - EARLY DOWN ONLY
# 3 - LONGER YARDAGE, 1ST - 3RD
# 4 - SHORT YARDAGE ONLY
# 5 - 2ND / 3RD DOWN
# 6 - SUPER MID


detach("file:NEW_pff_rushing_all_clusters_final_workspace.RData")
...
rushing_stats_final$rush_prop = rushing_stats_final$rushes / rushing_stats_final$team_rushes
rushing_stats_final$pbp_xtd_prop = rushing_stats_final$pbp_xtds / rushing_stats_final$team_pbp_xtds


rushing_stats_final %>% 
  filter(rusher_player_name == "S.Barkley", qbgrp_ssn == "PHIHurts-2024")

rushing_stats_final %>% 
  filter(rank_grp == "A" & qbgrp_ssn %in% c("KCMahomes-2024", "KCMahomes-2023"))
...
rush_func <- function(qbgrp_one, defgrp_one, rank_grp_input, run_situation_input, 
                     run_gap_input, zone_perc_range, player_name) {
  
  wb_tds <- createWorkbook()
  
  # Define a list of categories with thresholds and functions
  categories <- list(
    blitz = list(qb_threshold = .835, def_threshold = .885, qb_func = comparison_blitz_func, def_func = comparison_blitz_def_func),
    depth = list(qb_threshold = .82, def_threshold = .88, qb_func = comparison_depth_func, def_func = comparison_depth_def_func),
    less = list(qb_threshold = .78, def_threshold = .83, qb_func = comparison_less_func, def_func = comparison_less_def_func),
    pa = list(qb_threshold = .84, def_threshold = .885, qb_func = comparison_pa_func, def_func = comparison_pa_def_func),
    pressure = list(qb_threshold = .875, def_threshold = .91, qb_func = comparison_pressure_func, def_func = comparison_pressure_def_func)
  )
  
  process_category <- function(category, qbgrp_one, defgrp_one) {
    qb_teams <- c(category$qb_func(qbgrp_one, category$qb_threshold)$Team, qbgrp_one)
    def_teams <- c(category$def_func(defgrp_one, category$def_threshold)$Team, defgrp_one)
    
    rushing_stats_final %>%
      ungroup() %>%  # Ensure no residual grouping
      filter(qbgrp_ssn %in% qb_teams) %>%
      mutate(
        ind = ifelse(def_ssn %in% def_teams, "In", "Out"),
        rush_ind = ifelse(
          rank_grp == rank_grp_input & 
            run_situation_cluster %in% run_situation_input &
            run_gap_cluster %in% run_gap_input & 
            zone_perc_z_score >= zone_perc_range[1] &  
            zone_perc_z_score <= zone_perc_range[2], 
          "In", "Out")
      ) %>%
      mutate(has_rel_rushers = any(rush_ind == "In")) %>%
      filter(has_rel_rushers) %>%
      group_by(qbgrp_ssn, def_ssn, game_id, ind) %>%
      filter(sum(ifelse(rush_ind == "In", 1, 0), na.rm = TRUE) > 0) %>%
      dplyr::summarise(
        rel_players = sum(ifelse(rush_ind == "In", 1, 0), na.rm = TRUE),
        notrel_players = sum(ifelse(rush_ind != "In", 1, 0), na.rm = TRUE),
        rel_rushes = sum(ifelse(rush_ind == "In", rushes, 0), na.rm = TRUE),
        notrel_rushes = sum(ifelse(rush_ind != "In", rushes, 0), na.rm = TRUE),
        rel_pbp_xtds = sum(ifelse(rush_ind == "In", pbp_xtds, 0), na.rm = TRUE),
        notrel_pbp_xtds = sum(ifelse(rush_ind != "In", pbp_xtds, 0), na.rm = TRUE),
        rel_part_xtds = sum(ifelse(rush_ind == "In", part_xtds, 0), na.rm = TRUE),
        notrel_part_xtds = sum(ifelse(rush_ind != "In", part_xtds, 0), na.rm = TRUE),
  ...
  # Save workbook
  saveWorkbook(wb_tds, file.path("C:/Users/vflre/Downloads/NFL Models", paste0(sheet_name, ".xlsx")), overwrite = TRUE)
}

rush_func("PHIHurts-2024", "KC2024", rank_grp_input = "C", run_situation_input = c(3), run_gap_input = c(6), zone_perc_range = c(-.7, .6), player_name = "K.Gainwell")
